...
# Integration tests for grafana-mcp cartridge.
set -euo pipefail
...
echo "=== grafana-mcp integration tests ==="
...
if command -v idris2 &>/dev/null; then
    cd "$CART_DIR/abi" && idris2 --check GrafanaMcp.SafeRegistry 2>&1
    echo "  ABI: OK"
else
...
echo ""
echo "All tests passed for grafana-mcp!"
