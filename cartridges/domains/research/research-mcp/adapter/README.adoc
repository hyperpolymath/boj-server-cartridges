...
= research-mcp / adapter — protocol bridge
:orientation: shallow
...
| `research_adapter.zig` | Protocol dispatch (129 lines). Tool catalogue matches `cartridge.json`.
...
. `research_adapter.zig` — dispatch function and per-protocol routers.
