...
= redis-mcp / adapter — protocol bridge
:orientation: shallow
...
| `redis_mcp_adapter.zig` | Protocol dispatch (118 lines). Tool catalogue matches `cartridge.json`.
...
. `redis_mcp_adapter.zig` — dispatch function and per-protocol routers.
