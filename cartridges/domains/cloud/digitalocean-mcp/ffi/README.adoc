...
= digitalocean-mcp / ffi — Zig FFI layer
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libdigitalocean.so`.
| `digitalocean_mcp_ffi.zig`     | C-ABI exports (10 exports, 6 inline tests, 324 lines).
|===
...
6 inline `test "..."` block(s) in `digitalocean_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `digitalocean_mcp_ffi.zig` — C-ABI signatures and guards.
