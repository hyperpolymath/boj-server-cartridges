...
# Integration tests for npm-registry-mcp cartridge.
set -euo pipefail
...
echo "=== npm-registry-mcp integration tests ==="
...
if command -v idris2 &>/dev/null; then
    cd "$CART_DIR/abi" && idris2 --check NpmRegistryMcp.SafeRegistry 2>&1
    echo "  ABI: OK"
else
...
echo ""
echo "All tests passed for npm-registry-mcp!"
