...
= linode-mcp / adapter — protocol bridge
:orientation: shallow
...
| `linode_adapter.zig` | Protocol dispatch (236 lines). Tool catalogue matches `cartridge.json`.
...
. `linode_adapter.zig` — dispatch function and per-protocol routers.
