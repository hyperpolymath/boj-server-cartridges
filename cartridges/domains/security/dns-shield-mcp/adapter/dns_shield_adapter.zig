...
const REST_PORT: u16 = 9226;
const GRPC_PORT: u16 = 9227;
const GQL_PORT:  u16 = 9228;
...
fn dispatch(tool: []const u8, _body: []const u8, resp: []u8) Response {
    _ = _body;
    if (std.mem.eql(u8, tool, "dns_resolve_doq")) {
        return .{ .status = 200, .body = okJson(resp, "dns_resolve_doq forwarded") };
    }
    if (std.mem.eql(u8, tool, "dns_resolve_doh")) {
        return .{ .status = 200, .body = okJson(resp, "dns_resolve_doh forwarded") };
    }
    if (std.mem.eql(u8, tool, "dns_check_caa")) {
        return .{ .status = 200, .body = okJson(resp, "dns_check_caa forwarded") };
    }
    if (std.mem.eql(u8, tool, "dns_validate_dnssec")) {
        return .{ .status = 200, .body = okJson(resp, "dns_validate_dnssec forwarded") };
    }
    if (std.mem.eql(u8, tool, "dns_flush_cache")) {
        return .{ .status = 200, .body = okJson(resp, "dns_flush_cache forwarded") };
    }
...
fn dispatchGraphql(body: []const u8, resp: []u8) Response {
    if (std.mem.indexOf(u8, body, "dns_resolve_doq") != null)
        return dispatch("dns_resolve_doq", body, resp);
    if (std.mem.indexOf(u8, body, "dns_resolve_doh") != null)
        return dispatch("dns_resolve_doh", body, resp);
    if (std.mem.indexOf(u8, body, "dns_check_caa") != null)
        return dispatch("dns_check_caa", body, resp);
    if (std.mem.indexOf(u8, body, "dns_validate_dnssec") != null)
        return dispatch("dns_validate_dnssec", body, resp);
    if (std.mem.indexOf(u8, body, "dns_flush_cache") != null)
        return dispatch("dns_flush_cache", body, resp);
