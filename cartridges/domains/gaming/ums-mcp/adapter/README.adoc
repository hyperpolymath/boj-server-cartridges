...
= ums-mcp / adapter — protocol bridge
:orientation: shallow
...
| `ums_adapter.zig` | Protocol dispatch (144 lines). Tool catalogue matches `cartridge.json`.
...
. `ums_adapter.zig` — dispatch function and per-protocol routers.
