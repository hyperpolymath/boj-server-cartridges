...
# Integration tests for fly-mcp cartridge.
set -euo pipefail
...
echo "=== fly-mcp integration tests ==="
...
if command -v idris2 &>/dev/null; then
    cd "$CART_DIR/abi" && idris2 --check FlyMcp.SafeCloud 2>&1
    echo "  ABI: OK"
else
...
echo ""
echo "All tests passed for fly-mcp!"
