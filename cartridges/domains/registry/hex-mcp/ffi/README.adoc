...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libhex.so`.
| `hex_mcp_ffi.zig`     | C-ABI exports (16 exports, 7 inline tests, 403 lines).
|===
...
7 inline `test "..."` block(s) in `hex_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `hex_mcp_ffi.zig` — C-ABI signatures and guards.
