...
= conflow-mcp / ffi — Zig FFI layer **(stub)**
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libconflow.so`.
| `conflow_ffi.zig`     | C-ABI exports (4 exports, 3 inline tests, 44 lines).
|===
...
3 inline `test "..."` block(s) in `conflow_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `conflow_ffi.zig` — C-ABI signatures and guards.
