...
//
// idaptik-admin-mcp/adapter/build.zig
...
pub fn build(b: *std.Build) void {
    ...
    const ffi_mod = b.createModule(.{
        .root_source_file = b.path("../ffi/idaptik_admin_ffi.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const adapter_mod = b.createModule(.{
        .root_source_file = b.path("idaptik_admin_adapter.zig"),
        .target = target,
        .optimize = optimize,
    });
    adapter_mod.addImport("idaptik_admin_ffi", ffi_mod);

    const adapter = b.addExecutable(.{
        .name = "idaptik_admin_adapter",
        .root_module = adapter_mod,
    });
    b.installArtifact(adapter);

    const run_step = b.step("run", "Run the idaptik-admin-mcp adapter");
    ...
    const test_step = b.step("test", "Run idaptik-admin-mcp adapter tests");
