...
| `build.zig`      | Zig build graph — produces `zig-out/lib/librokur.so`.
| `rokur_mcp_ffi.zig`     | C-ABI exports (13 exports, 5 inline tests, 387 lines).
|===
...
5 inline `test "..."` block(s) in `rokur_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `rokur_mcp_ffi.zig` — C-ABI signatures and guards.
