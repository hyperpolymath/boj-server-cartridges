...
# Integration tests for neon-mcp cartridge.
set -euo pipefail
...
echo "=== neon-mcp integration tests ==="
...
if command -v idris2 &>/dev/null; then
    cd "$CART_DIR/abi" && idris2 --check NeonMcp.SafeDatabase 2>&1
    echo "  ABI: OK"
else
...
echo ""
echo "All tests passed for neon-mcp!"
