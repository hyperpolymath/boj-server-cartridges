...
= opam-mcp / adapter — protocol bridge
:orientation: shallow
...
| `opam_adapter.zig` | Protocol dispatch (204 lines). Tool catalogue matches `cartridge.json`.
...
. `opam_adapter.zig` — dispatch function and per-protocol routers.
