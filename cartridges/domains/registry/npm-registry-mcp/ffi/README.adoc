...
= npm-registry-mcp / ffi — Zig FFI layer
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libnpm-registry.so`.
| `npm_registry_mcp_ffi.zig`     | C-ABI exports (15 exports, 7 inline tests, 427 lines).
|===
...
7 inline `test "..."` block(s) in `npm_registry_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `npm_registry_mcp_ffi.zig` — C-ABI signatures and guards.
