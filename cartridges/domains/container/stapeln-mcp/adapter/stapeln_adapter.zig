...
// Tools: stapeln_list_stacks, stapeln_deploy, stapeln_scale, stapeln_get_health

const std = @import("std");
const ffi = @import("stapeln_ffi");

const REST_PORT: u16 = 9112;
const GRPC_PORT: u16 = 9113;
const GQL_PORT:  u16 = 9114;
...
fn statusJson(buf: []u8) []u8 {
    const n = std.fmt.bufPrint(buf, "{{\"success\":true,\"state\":\"ready\",\"service\":\"stapeln-mcp\"}}", .{}) catch return buf[0..0];
    return n;
}
...
fn dispatch(tool: []const u8, body: []const u8, resp: []u8) Response {
    _ = body;
    if (std.mem.eql(u8, tool, "stapeln_list_stacks")) return .{ .status = 200, .body = okJson(resp, "stapeln_list_stacks forwarded") };
    if (std.mem.eql(u8, tool, "stapeln_deploy")) return .{ .status = 200, .body = okJson(resp, "stapeln_deploy forwarded") };
    if (std.mem.eql(u8, tool, "stapeln_scale")) return .{ .status = 200, .body = okJson(resp, "stapeln_scale forwarded") };
    if (std.mem.eql(u8, tool, "stapeln_get_health")) return .{ .status = 200, .body = okJson(resp, "stapeln_get_health forwarded") };
...
fn dispatchGrpc(path: []const u8, body: []const u8, resp: []u8) Response {
    const prefix = "/Stapelnservice/";
    if (!std.mem.startsWith(u8, path, prefix))
        return .{ .status = 404, .body = errJson(resp, "Not a recognized gRPC path") };
    const method = path[prefix.len..];
    const tool = blk: {
        if (std.mem.eql(u8, method, "stapeln_list_stacks")) break :blk "stapeln_list_stacks";
        if (std.mem.eql(u8, method, "stapeln_deploy")) break :blk "stapeln_deploy";
        if (std.mem.eql(u8, method, "stapeln_scale")) break :blk "stapeln_scale";
        if (std.mem.eql(u8, method, "stapeln_get_health")) break :blk "stapeln_get_health";
...
fn dispatchGraphql(body: []const u8, resp: []u8) Response {
    ...
    if (std.mem.indexOf(u8, body, "list_stacks") != null) return dispatch("stapeln_list_stacks", body, resp);
    if (std.mem.indexOf(u8, body, "deploy") != null) return dispatch("stapeln_deploy", body, resp);
    if (std.mem.indexOf(u8, body, "scale") != null) return dispatch("stapeln_scale", body, resp);
    if (std.mem.indexOf(u8, body, "get_health") != null) return dispatch("stapeln_get_health", body, resp);
