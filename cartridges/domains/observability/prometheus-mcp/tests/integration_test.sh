...
# Integration tests for prometheus-mcp cartridge.
set -euo pipefail
...
echo "=== prometheus-mcp integration tests ==="
...
if command -v idris2 &>/dev/null; then
    cd "$CART_DIR/abi" && idris2 --check PrometheusMcp.SafeRegistry 2>&1
    echo "  ABI: OK"
else
...
echo ""
echo "All tests passed for prometheus-mcp!"
