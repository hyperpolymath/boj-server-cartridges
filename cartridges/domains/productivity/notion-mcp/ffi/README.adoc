...
= notion-mcp / ffi — Zig FFI layer **(stub)**
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libnotion.so`.
| `notion_mcp_ffi.zig`     | C-ABI exports (11 exports, 6 inline tests, 540 lines).
|===
...
6 inline `test "..."` block(s) in `notion_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `notion_mcp_ffi.zig` — C-ABI signatures and guards.
