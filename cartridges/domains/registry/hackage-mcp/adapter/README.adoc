...
= hackage-mcp / adapter — protocol bridge
:orientation: shallow
...
| `hackage_adapter.zig` | Protocol dispatch (212 lines). Tool catalogue matches `cartridge.json`.
...
. `hackage_adapter.zig` — dispatch function and per-protocol routers.
