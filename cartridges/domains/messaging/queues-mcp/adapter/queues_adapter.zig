...
//
// queues-mcp/adapter/queues_adapter.zig
//
// Three-protocol BoJ adapter: REST (port 9280), gRPC-compat (port 9281),
// GraphQL (port 9282).
// Replaces the banned zig adapter (queues_adapter.v).

const std = @import("std");
const ffi = @import("queues_ffi");

const REST_PORT: u16 = 9280;
const GRPC_PORT: u16 = 9281;
const GQL_PORT:  u16 = 9282;
...
fn dispatch(tool: []const u8, _body: []const u8, resp: []u8) Response {
    _ = _body;
    if (std.mem.eql(u8, tool, "queue_connect")) {
        return .{ .status = 200, .body = okJson(resp, "queue_connect forwarded") };
    }
    if (std.mem.eql(u8, tool, "queue_publish")) {
        return .{ .status = 200, .body = okJson(resp, "queue_publish forwarded") };
    }
    if (std.mem.eql(u8, tool, "queue_subscribe")) {
        return .{ .status = 200, .body = okJson(resp, "queue_subscribe forwarded") };
    }
    if (std.mem.eql(u8, tool, "queue_consume")) {
        return .{ .status = 200, .body = okJson(resp, "queue_consume forwarded") };
    }
    if (std.mem.eql(u8, tool, "queue_ack")) {
        return .{ .status = 200, .body = okJson(resp, "queue_ack forwarded") };
    }
    if (std.mem.eql(u8, tool, "queue_disconnect")) {
        return .{ .status = 200, .body = okJson(resp, "queue_disconnect forwarded") };
    }
...
fn dispatchGraphql(body: []const u8, resp: []u8) Response {
    if (std.mem.indexOf(u8, body, "queue_connect") != null)
        return dispatch("queue_connect", body, resp);
    if (std.mem.indexOf(u8, body, "queue_publish") != null)
        return dispatch("queue_publish", body, resp);
    if (std.mem.indexOf(u8, body, "queue_subscribe") != null)
        return dispatch("queue_subscribe", body, resp);
    if (std.mem.indexOf(u8, body, "queue_consume") != null)
        return dispatch("queue_consume", body, resp);
    if (std.mem.indexOf(u8, body, "queue_ack") != null)
        return dispatch("queue_ack", body, resp);
    if (std.mem.indexOf(u8, body, "queue_disconnect") != null)
        return dispatch("queue_disconnect", body, resp);
