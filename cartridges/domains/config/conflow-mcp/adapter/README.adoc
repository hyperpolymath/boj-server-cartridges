...
= conflow-mcp / adapter — protocol bridge
:orientation: shallow
...
| `conflow_adapter.zig` | Protocol dispatch (129 lines). Tool catalogue matches `cartridge.json`.
...
. `conflow_adapter.zig` — dispatch function and per-protocol routers.
