...
# Integration tests for slack-mcp cartridge.
set -euo pipefail
...
echo "=== slack-mcp integration tests ==="
...
if command -v idris2 &>/dev/null; then
    cd "$CART_DIR/abi" && idris2 --check SlackMcp.SafeComms 2>&1
    echo "  ABI: OK"
else
...
echo ""
echo "All tests passed for slack-mcp!"
