...
//
// conflow-mcp/adapter/conflow_adapter.zig
//
// Three-protocol BoJ adapter: REST (port 9220), gRPC-compat (port 9221),
// GraphQL (port 9222).
// Replaces the banned zig adapter (conflow_adapter.v).

const std = @import("std");
const ffi = @import("conflow_ffi");

const REST_PORT: u16 = 9220;
const GRPC_PORT: u16 = 9221;
const GQL_PORT:  u16 = 9222;
...
fn dispatch(tool: []const u8, _body: []const u8, resp: []u8) Response {
    _ = _body;
    if (std.mem.eql(u8, tool, "conflow_get_config")) {
        return .{ .status = 200, .body = okJson(resp, "conflow_get_config forwarded") };
    }
    if (std.mem.eql(u8, tool, "conflow_apply_config")) {
        return .{ .status = 200, .body = okJson(resp, "conflow_apply_config forwarded") };
    }
    if (std.mem.eql(u8, tool, "conflow_validate_config")) {
        return .{ .status = 200, .body = okJson(resp, "conflow_validate_config forwarded") };
    }
    if (std.mem.eql(u8, tool, "conflow_diff_config")) {
        return .{ .status = 200, .body = okJson(resp, "conflow_diff_config forwarded") };
    }
...
fn dispatchGraphql(body: []const u8, resp: []u8) Response {
    if (std.mem.indexOf(u8, body, "conflow_get_config") != null)
        return dispatch("conflow_get_config", body, resp);
    if (std.mem.indexOf(u8, body, "conflow_apply_config") != null)
        return dispatch("conflow_apply_config", body, resp);
    if (std.mem.indexOf(u8, body, "conflow_validate_config") != null)
        return dispatch("conflow_validate_config", body, resp);
    if (std.mem.indexOf(u8, body, "conflow_diff_config") != null)
        return dispatch("conflow_diff_config", body, resp);
