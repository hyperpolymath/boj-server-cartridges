...
= aws-mcp / adapter — protocol bridge
:orientation: shallow
...
| `aws_mcp_adapter.zig` | Protocol dispatch (115 lines). Tool catalogue matches `cartridge.json`.
...
. `aws_mcp_adapter.zig` — dispatch function and per-protocol routers.
