...
= stapeln-mcp / adapter — protocol bridge
:orientation: shallow
...
| `stapeln_adapter.zig` | Protocol dispatch (121 lines). Tool catalogue matches `cartridge.json`.
...
. `stapeln_adapter.zig` — dispatch function and per-protocol routers.
