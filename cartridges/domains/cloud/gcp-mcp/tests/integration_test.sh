...
# Integration tests for gcp-mcp cartridge.
set -euo pipefail
...
echo "=== gcp-mcp integration tests ==="
...
if command -v idris2 &>/dev/null; then
    cd "$CART_DIR/abi" && idris2 --check GcpMcp.SafeCloud 2>&1
    echo "  ABI: OK"
else
...
echo ""
echo "All tests passed for gcp-mcp!"
