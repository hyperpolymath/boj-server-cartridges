...
# Integration tests for zotero-mcp cartridge.
set -euo pipefail
...
echo "=== zotero-mcp integration tests ==="
...
if command -v idris2 &>/dev/null; then
    cd "$CART_DIR/abi" && idris2 --check ZoteroMcp.SafeRegistry 2>&1
    echo "  ABI: OK"
else
...
echo ""
echo "All tests passed for zotero-mcp!"
