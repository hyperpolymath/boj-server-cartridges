...
# Integration tests for crates-mcp cartridge.
set -euo pipefail
...
echo "=== crates-mcp integration tests ==="
...
if command -v idris2 &>/dev/null; then
    cd "$CART_DIR/abi" && idris2 --check CratesMcp.SafeRegistry 2>&1
    echo "  ABI: OK"
else
...
echo ""
echo "All tests passed for crates-mcp!"
