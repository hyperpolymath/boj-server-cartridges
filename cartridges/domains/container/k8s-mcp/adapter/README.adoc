...
= k8s-mcp / adapter — protocol bridge
:orientation: shallow
...
| `k8s_adapter.zig` | Protocol dispatch (115 lines). Tool catalogue matches `cartridge.json`.
...
. `k8s_adapter.zig` — dispatch function and per-protocol routers.
