...
# Integration tests for pypi-mcp cartridge.
set -euo pipefail
...
echo "=== pypi-mcp integration tests ==="
...
if command -v idris2 &>/dev/null; then
    cd "$CART_DIR/abi" && idris2 --check PypiMcp.SafeRegistry 2>&1
    echo "  ABI: OK"
else
...
echo ""
echo "All tests passed for pypi-mcp!"
