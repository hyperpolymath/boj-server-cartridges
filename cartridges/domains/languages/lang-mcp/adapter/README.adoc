...
= lang-mcp / adapter — protocol bridge
:orientation: shallow
...
| `lang_adapter.zig` | Protocol dispatch (499 lines). Tool catalogue matches `cartridge.json`.
...
. `lang_adapter.zig` — dispatch function and per-protocol routers.
