...
= cloudflare-mcp / ffi — Zig FFI layer
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libcloudflare.so`.
| `cloudflare_mcp_ffi.zig`     | C-ABI exports (6 exports, 0
0 inline tests, 138 lines).
|===
...
0
0 inline `test "..."` block(s) in `cloudflare_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `cloudflare_mcp_ffi.zig` — C-ABI signatures and guards.
