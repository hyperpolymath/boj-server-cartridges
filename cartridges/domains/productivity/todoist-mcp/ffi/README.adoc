...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libtodoist.so`.
| `todoist_mcp_ffi.zig`     | C-ABI exports (15 exports, 5 inline tests, 353 lines).
|===
...
5 inline `test "..."` block(s) in `todoist_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `todoist_mcp_ffi.zig` — C-ABI signatures and guards.
