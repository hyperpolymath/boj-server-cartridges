...
= panic-attack-mcp / adapter — protocol bridge
:orientation: shallow
...
| `panic_attack_adapter.zig` | Protocol dispatch (118 lines). Tool catalogue matches `cartridge.json`.
...
. `panic_attack_adapter.zig` — dispatch function and per-protocol routers.
