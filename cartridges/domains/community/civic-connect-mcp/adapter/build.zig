...
//
// civic-connect-mcp/adapter/build.zig
...
pub fn build(b: *std.Build) void {
    ...
    const ffi_mod = b.createModule(.{
        .root_source_file = b.path("../ffi/civic_connect_ffi.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const adapter_mod = b.createModule(.{
        .root_source_file = b.path("civic_connect_adapter.zig"),
        .target = target,
        .optimize = optimize,
    });
    adapter_mod.addImport("civic_connect_ffi", ffi_mod);

    const adapter = b.addExecutable(.{
        .name = "civic_connect_adapter",
        .root_module = adapter_mod,
    });
    b.installArtifact(adapter);

    const run_step = b.step("run", "Run the civic-connect-mcp adapter");
    ...
    const test_step = b.step("test", "Run civic-connect-mcp adapter tests");
