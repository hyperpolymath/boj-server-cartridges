...
//
// codeseeker-mcp/adapter/codeseeker_adapter.zig
//
// Three-protocol BoJ adapter: REST (port 9214), gRPC-compat (port 9215),
// GraphQL (port 9216).
// Replaces the banned zig adapter (codeseeker_adapter.v).

const std = @import("std");
const ffi = @import("codeseeker_ffi");

const REST_PORT: u16 = 9214;
const GRPC_PORT: u16 = 9215;
const GQL_PORT:  u16 = 9216;
...
fn dispatch(tool: []const u8, _body: []const u8, resp: []u8) Response {
    _ = _body;
    if (std.mem.eql(u8, tool, "codeseeker_open")) {
        return .{ .status = 200, .body = okJson(resp, "codeseeker_open forwarded") };
    }
    if (std.mem.eql(u8, tool, "codeseeker_close")) {
        return .{ .status = 200, .body = okJson(resp, "codeseeker_close forwarded") };
    }
    if (std.mem.eql(u8, tool, "codeseeker_index")) {
        return .{ .status = 200, .body = okJson(resp, "codeseeker_index forwarded") };
    }
    if (std.mem.eql(u8, tool, "codeseeker_query")) {
        return .{ .status = 200, .body = okJson(resp, "codeseeker_query forwarded") };
    }
    if (std.mem.eql(u8, tool, "codeseeker_state")) {
        return .{ .status = 200, .body = okJson(resp, "codeseeker_state forwarded") };
    }
...
fn dispatchGraphql(body: []const u8, resp: []u8) Response {
    if (std.mem.indexOf(u8, body, "codeseeker_open") != null)
        return dispatch("codeseeker_open", body, resp);
    if (std.mem.indexOf(u8, body, "codeseeker_close") != null)
        return dispatch("codeseeker_close", body, resp);
    if (std.mem.indexOf(u8, body, "codeseeker_index") != null)
        return dispatch("codeseeker_index", body, resp);
    if (std.mem.indexOf(u8, body, "codeseeker_query") != null)
        return dispatch("codeseeker_query", body, resp);
    if (std.mem.indexOf(u8, body, "codeseeker_state") != null)
        return dispatch("codeseeker_state", body, resp);
