...
= typed-wasm-mcp / ffi — Zig FFI layer **(stub)**
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libtyped-wasm.so`.
| `typed_wasm_ffi.zig`     | C-ABI exports (3 exports, 3 inline tests, 40 lines).
|===
...
3 inline `test "..."` block(s) in `typed_wasm_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `typed_wasm_ffi.zig` — C-ABI signatures and guards.
