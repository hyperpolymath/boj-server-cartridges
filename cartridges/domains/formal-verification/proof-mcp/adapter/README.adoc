...
= proof-mcp / adapter — protocol bridge
:orientation: shallow
...
| `proof_adapter.zig` | Protocol dispatch (133 lines). Tool catalogue matches `cartridge.json`.
...
. `proof_adapter.zig` — dispatch function and per-protocol routers.
