...
pub fn build(b: *std.Build) void {
    ...
    const ffi_mod = b.createModule(.{
        .root_source_file = b.path("../ffi/duckdb_mcp_ffi.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const adapter_mod = b.createModule(.{
        .root_source_file = b.path("duckdb_mcp_adapter.zig"),
        .target = target,
        .optimize = optimize,
    });
    adapter_mod.addImport("duckdb_mcp_ffi", ffi_mod);

    const adapter = b.addExecutable(.{
        .name = "duckdb_mcp_adapter",
        .root_module = adapter_mod,
    });
    b.installArtifact(adapter);

    const run_step = b.step("run", "Run the duckdb-mcp adapter");
    ...
    const test_step = b.step("test", "Run duckdb-mcp adapter tests");
