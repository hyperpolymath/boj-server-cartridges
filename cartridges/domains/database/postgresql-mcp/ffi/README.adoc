...
= postgresql-mcp / ffi — Zig FFI layer
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libpostgresql.so`.
| `postgresql_mcp_ffi.zig`     | C-ABI exports (12 exports, 4 inline tests, 366 lines).
|===
...
4 inline `test "..."` block(s) in `postgresql_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `postgresql_mcp_ffi.zig` — C-ABI signatures and guards.
