...
= container-mcp / ffi — Zig FFI layer
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libcontainer.so`.
| `container_ffi.zig`     | C-ABI exports (12 exports, 7 inline tests, 280 lines).
|===
...
7 inline `test "..."` block(s) in `container_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `container_ffi.zig` — C-ABI signatures and guards.
