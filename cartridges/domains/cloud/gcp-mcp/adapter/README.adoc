...
= gcp-mcp / adapter — protocol bridge
:orientation: shallow
...
| `gcp_mcp_adapter.zig` | Protocol dispatch (112 lines). Tool catalogue matches `cartridge.json`.
...
. `gcp_mcp_adapter.zig` — dispatch function and per-protocol routers.
