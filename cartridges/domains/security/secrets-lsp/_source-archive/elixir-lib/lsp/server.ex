...
defmodule PolySecret.LSP.Server do
  @moduledoc """
  GenLSP server implementation for PolySecret.
  ...
  alias PolySecret.LSP.Handlers.{Completion, Diagnostics, Hover}
  ...
  def init(_lsp, _args) do
    {:ok, %{project_path: nil, detected_secret_manager: nil, documents: %{}}}
  end
  ...
  def handle_request(%{"method" => "initialize", "params" => params}, lsp) do
    ...
    # Auto-detect secret manager
    detected_secret_manager = detect_secret_manager(project_path)

    Logger.info("Detected secret manager: #{inspect(detected_secret_manager)}")
    ...
    server_capabilities = %{
      ...
      "executeCommandProvider" => %{
        "commands" => ["poly-secret.validate", "poly-secret.check-encryption"]
      }
    }
    ...
    result = %{
      ...
      "serverInfo" => %{
        "name" => "PolySecret LSP",
        "version" => PolySecret.LSP.version()
      }
    }

    new_state = Map.merge(lsp, %{project_path: project_path, detected_secret_manager: detected_secret_manager})
  ...
  defp detect_secret_manager(nil), do: nil
  defp detect_secret_manager(project_path) do
    adapters = [
      {PolySecret.Adapters.Vault, :vault},
      {PolySecret.Adapters.SOPS, :sops}
    ]
  ...
  defp execute_command("poly-secret.validate", _args, %{project_path: path, detected_secret_manager: manager}) when path != nil do
    case manager do
      :vault -> PolySecret.Adapters.Vault.validate(path, [])
      :sops -> PolySecret.Adapters.SOPS.validate(path, [])
      _ -> {:error, "No secret manager detected"}
    end
  end

  defp execute_command("poly-secret.check-encryption", _args, %{project_path: path, detected_secret_manager: manager}) when path != nil do
    case manager do
      :vault -> {:ok, "Vault uses server-side encryption"}
      :sops -> PolySecret.Adapters.SOPS.check_encryption(path, [])
      _ -> {:error, "No secret manager detected"}
    end
  end
