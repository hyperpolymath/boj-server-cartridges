...
= slack-mcp / adapter — protocol bridge
:orientation: shallow
...
| `slack_mcp_adapter.zig` | Protocol dispatch (112 lines). Tool catalogue matches `cartridge.json`.
...
. `slack_mcp_adapter.zig` — dispatch function and per-protocol routers.
