...
= obsidian-mcp / adapter — protocol bridge
:orientation: shallow
...
| `obsidian_adapter.zig` | Protocol dispatch (212 lines). Tool catalogue matches `cartridge.json`.
...
. `obsidian_adapter.zig` — dispatch function and per-protocol routers.
