...
= neo4j-mcp / adapter — protocol bridge
:orientation: shallow
...
| `neo4j_mcp_adapter.zig` | Protocol dispatch (134 lines). Tool catalogue matches `cartridge.json`.
...
. `neo4j_mcp_adapter.zig` — dispatch function and per-protocol routers.
