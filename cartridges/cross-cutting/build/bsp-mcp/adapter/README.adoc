...
= bsp-mcp / adapter — protocol bridge
:orientation: shallow
...
| `bsp_adapter.zig` | Protocol dispatch (414 lines). Tool catalogue matches `cartridge.json`.
|===
...
. `bsp_adapter.zig` — dispatch function and per-protocol routers.
