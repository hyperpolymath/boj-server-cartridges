...
= buildkite-mcp / ffi — Zig FFI layer
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libbuildkite.so`.
| `buildkite_mcp_ffi.zig`     | C-ABI exports (14 exports, 5 inline tests, 272 lines).
|===
...
5 inline `test "..."` block(s) in `buildkite_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `buildkite_mcp_ffi.zig` — C-ABI signatures and guards.
