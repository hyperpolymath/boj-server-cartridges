...
= mongodb-mcp / adapter — protocol bridge
:orientation: shallow
...
| `mongodb_mcp_adapter.zig` | Protocol dispatch (115 lines). Tool catalogue matches `cartridge.json`.
...
. `mongodb_mcp_adapter.zig` — dispatch function and per-protocol routers.
