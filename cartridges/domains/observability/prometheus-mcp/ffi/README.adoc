...
= prometheus-mcp / ffi — Zig FFI layer
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libprometheus.so`.
| `prometheus_mcp_ffi.zig`     | C-ABI exports (15 exports, 6 inline tests, 356 lines).
|===
...
6 inline `test "..."` block(s) in `prometheus_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `prometheus_mcp_ffi.zig` — C-ABI signatures and guards.
