...
// Tools: verisimdb_store_octad, verisimdb_get_octad, verisimdb_detect_drift, verisimdb_query_audit

const std = @import("std");
const ffi = @import("verisimdb_ffi");

const REST_PORT: u16 = 9115;
const GRPC_PORT: u16 = 9116;
const GQL_PORT:  u16 = 9117;
...
fn statusJson(buf: []u8) []u8 {
    const n = std.fmt.bufPrint(buf, "{{\"success\":true,\"state\":\"ready\",\"service\":\"verisimdb-mcp\"}}", .{}) catch return buf[0..0];
    return n;
}
...
fn dispatch(tool: []const u8, body: []const u8, resp: []u8) Response {
    _ = body;
    if (std.mem.eql(u8, tool, "verisimdb_store_octad")) return .{ .status = 200, .body = okJson(resp, "verisimdb_store_octad forwarded") };
    if (std.mem.eql(u8, tool, "verisimdb_get_octad")) return .{ .status = 200, .body = okJson(resp, "verisimdb_get_octad forwarded") };
    if (std.mem.eql(u8, tool, "verisimdb_detect_drift")) return .{ .status = 200, .body = okJson(resp, "verisimdb_detect_drift forwarded") };
    if (std.mem.eql(u8, tool, "verisimdb_query_audit")) return .{ .status = 200, .body = okJson(resp, "verisimdb_query_audit forwarded") };
...
fn dispatchGrpc(path: []const u8, body: []const u8, resp: []u8) Response {
    const prefix = "/Verisimdbservice/";
    if (!std.mem.startsWith(u8, path, prefix))
        return .{ .status = 404, .body = errJson(resp, "Not a recognized gRPC path") };
    const method = path[prefix.len..];
    const tool = blk: {
        if (std.mem.eql(u8, method, "verisimdb_store_octad")) break :blk "verisimdb_store_octad";
        if (std.mem.eql(u8, method, "verisimdb_get_octad")) break :blk "verisimdb_get_octad";
        if (std.mem.eql(u8, method, "verisimdb_detect_drift")) break :blk "verisimdb_detect_drift";
        if (std.mem.eql(u8, method, "verisimdb_query_audit")) break :blk "verisimdb_query_audit";
...
fn dispatchGraphql(body: []const u8, resp: []u8) Response {
    ...
    if (std.mem.indexOf(u8, body, "store_octad") != null) return dispatch("verisimdb_store_octad", body, resp);
    if (std.mem.indexOf(u8, body, "get_octad") != null) return dispatch("verisimdb_get_octad", body, resp);
    if (std.mem.indexOf(u8, body, "detect_drift") != null) return dispatch("verisimdb_detect_drift", body, resp);
    if (std.mem.indexOf(u8, body, "query_audit") != null) return dispatch("verisimdb_query_audit", body, resp);
