...
= npm-registry-mcp / adapter — protocol bridge
:orientation: shallow
...
| `npm_registry_adapter.zig` | Protocol dispatch (212 lines). Tool catalogue matches `cartridge.json`.
...
. `npm_registry_adapter.zig` — dispatch function and per-protocol routers.
