...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libcrates.so`.
| `crates_mcp_ffi.zig`     | C-ABI exports (15 exports, 7 inline tests, 427 lines).
|===
...
7 inline `test "..."` block(s) in `crates_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `crates_mcp_ffi.zig` — C-ABI signatures and guards.
