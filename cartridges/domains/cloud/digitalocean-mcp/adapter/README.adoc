...
= digitalocean-mcp / adapter — protocol bridge
:orientation: shallow
...
| `digitalocean_mcp_adapter.zig` | Protocol dispatch (159 lines). Tool catalogue matches `cartridge.json`.
...
. `digitalocean_mcp_adapter.zig` — dispatch function and per-protocol routers.
