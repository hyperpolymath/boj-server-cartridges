...
= fly-mcp / adapter — protocol bridge
:orientation: shallow
...
| `fly_adapter.zig` | Protocol dispatch (248 lines). Tool catalogue matches `cartridge.json`.
...
. `fly_adapter.zig` — dispatch function and per-protocol routers.
