...
= codeseeker-mcp / ffi — Zig FFI layer
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libcodeseeker.so`.
| `codeseeker_ffi.zig`     | C-ABI exports (16 exports, 8 inline tests, 367 lines).
|===
...
8 inline `test "..."` block(s) in `codeseeker_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `codeseeker_ffi.zig` — C-ABI signatures and guards.
