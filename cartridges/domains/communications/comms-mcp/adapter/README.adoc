...
= comms-mcp / adapter — protocol bridge
:orientation: shallow
...
| `comms_adapter.zig` | Protocol dispatch (129 lines). Tool catalogue matches `cartridge.json`.
...
. `comms_adapter.zig` — dispatch function and per-protocol routers.
