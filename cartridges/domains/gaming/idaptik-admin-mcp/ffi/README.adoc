...
= idaptik-admin-mcp / ffi — Zig FFI layer **(stub)**
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libidaptik-admin.so`.
| `idaptik_admin_ffi.zig`     | C-ABI exports (2 exports, 2 inline tests, 51 lines).
|===
...
2 inline `test "..."` block(s) in `idaptik_admin_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `idaptik_admin_ffi.zig` — C-ABI signatures and guards.
