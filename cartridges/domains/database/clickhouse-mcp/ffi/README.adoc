...
= clickhouse-mcp / ffi — Zig FFI layer
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libclickhouse.so`.
| `clickhouse_mcp_ffi.zig`     | C-ABI exports (12 exports, 7 inline tests, 372 lines).
|===
...
7 inline `test "..."` block(s) in `clickhouse_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `clickhouse_mcp_ffi.zig` — C-ABI signatures and guards.
