...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libslack.so`.
| `slack_mcp_ffi.zig`     | C-ABI exports (13 exports, 7 inline tests, 753 lines).
|===
...
7 inline `test "..."` block(s) in `slack_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `slack_mcp_ffi.zig` — C-ABI signatures and guards.
