...
= game-admin-mcp / ffi — Zig FFI layer **(stub)**
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libgame-admin.so`.
| `game_admin_ffi.zig`     | C-ABI exports (3 exports, 2 inline tests, 62 lines).
|===
...
2 inline `test "..."` block(s) in `game_admin_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `game_admin_ffi.zig` — C-ABI signatures and guards.
