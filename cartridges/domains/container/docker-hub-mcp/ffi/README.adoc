...
= docker-hub-mcp / ffi — Zig FFI layer
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libdocker-hub.so`.
| `docker_hub_mcp_ffi.zig`     | C-ABI exports (11 exports, 6 inline tests, 351 lines).
|===
...
6 inline `test "..."` block(s) in `docker_hub_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `docker_hub_mcp_ffi.zig` — C-ABI signatures and guards.
