...
//
// pmpl-mcp/adapter/pmpl_adapter.zig
//
// Three-protocol BoJ adapter: REST (port 9277), gRPC-compat (port 9278),
// GraphQL (port 9279).
// Replaces the banned zig adapter (pmpl_adapter.v).

const std = @import("std");
const ffi = @import("pmpl_ffi");

const REST_PORT: u16 = 9277;
const GRPC_PORT: u16 = 9278;
const GQL_PORT:  u16 = 9279;
...
fn dispatch(tool: []const u8, _body: []const u8, resp: []u8) Response {
    _ = _body;
    if (std.mem.eql(u8, tool, "pmpl_create_chain")) {
        return .{ .status = 200, .body = okJson(resp, "pmpl_create_chain forwarded") };
    }
    if (std.mem.eql(u8, tool, "pmpl_extend_chain")) {
        return .{ .status = 200, .body = okJson(resp, "pmpl_extend_chain forwarded") };
    }
    if (std.mem.eql(u8, tool, "pmpl_verify_chain")) {
        return .{ .status = 200, .body = okJson(resp, "pmpl_verify_chain forwarded") };
    }
    if (std.mem.eql(u8, tool, "pmpl_hash_artefact")) {
        return .{ .status = 200, .body = okJson(resp, "pmpl_hash_artefact forwarded") };
    }
    if (std.mem.eql(u8, tool, "pmpl_check_compatible")) {
        return .{ .status = 200, .body = okJson(resp, "pmpl_check_compatible forwarded") };
    }
...
fn dispatchGraphql(body: []const u8, resp: []u8) Response {
    if (std.mem.indexOf(u8, body, "pmpl_create_chain") != null)
        return dispatch("pmpl_create_chain", body, resp);
    if (std.mem.indexOf(u8, body, "pmpl_extend_chain") != null)
        return dispatch("pmpl_extend_chain", body, resp);
    if (std.mem.indexOf(u8, body, "pmpl_verify_chain") != null)
        return dispatch("pmpl_verify_chain", body, resp);
    if (std.mem.indexOf(u8, body, "pmpl_hash_artefact") != null)
        return dispatch("pmpl_hash_artefact", body, resp);
    if (std.mem.indexOf(u8, body, "pmpl_check_compatible") != null)
        return dispatch("pmpl_check_compatible", body, resp);
