...
= render-mcp / adapter — protocol bridge
:orientation: shallow
...
| `render_adapter.zig` | Protocol dispatch (228 lines). Tool catalogue matches `cartridge.json`.
...
. `render_adapter.zig` — dispatch function and per-protocol routers.
