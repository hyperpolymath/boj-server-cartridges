...
= vext-mcp / ffi — Zig FFI layer **(stub)**
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libvext.so`.
| `vext_ffi.zig`     | C-ABI exports (3 exports, 3 inline tests, 41 lines).
|===
...
3 inline `test "..."` block(s) in `vext_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `vext_ffi.zig` — C-ABI signatures and guards.
