...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libturso.so`.
| `turso_mcp_ffi.zig`     | C-ABI exports (10 exports, 5 inline tests, 292 lines).
|===
...
5 inline `test "..."` block(s) in `turso_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `turso_mcp_ffi.zig` — C-ABI signatures and guards.
