...
# Integration tests for hetzner-mcp cartridge.
set -euo pipefail
...
echo "=== hetzner-mcp integration tests ==="
...
if command -v idris2 &>/dev/null; then
    cd "$CART_DIR/abi" && idris2 --check HetznerMcp.SafeCloud 2>&1
    echo "  ABI: OK"
else
...
echo ""
echo "All tests passed for hetzner-mcp!"
