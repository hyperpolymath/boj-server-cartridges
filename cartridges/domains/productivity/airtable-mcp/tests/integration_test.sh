...
# Integration tests for airtable-mcp cartridge.
set -euo pipefail
...
echo "=== airtable-mcp integration tests ==="
...
if command -v idris2 &>/dev/null; then
    cd "$CART_DIR/abi" && idris2 --check AirtableMcp.SafeRegistry 2>&1
    echo "  ABI: OK"
else
...
echo ""
echo "All tests passed for airtable-mcp!"
