...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libsecrets.so`.
| `secrets_ffi.zig`     | C-ABI exports (13 exports, 6 inline tests, 270 lines).
|===
...
6 inline `test "..."` block(s) in `secrets_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `secrets_ffi.zig` — C-ABI signatures and guards.
