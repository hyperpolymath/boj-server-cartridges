...
= github-actions-mcp / adapter — protocol bridge
:orientation: shallow
...
| `github_actions_mcp_adapter.zig` | Protocol dispatch (144 lines). Tool catalogue matches `cartridge.json`.
...
. `github_actions_mcp_adapter.zig` — dispatch function and per-protocol routers.
