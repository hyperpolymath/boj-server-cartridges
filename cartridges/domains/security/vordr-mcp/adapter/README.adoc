...
= vordr-mcp / adapter — protocol bridge
:orientation: shallow
...
| `vordr_adapter.zig` | Protocol dispatch (121 lines). Tool catalogue matches `cartridge.json`.
...
. `vordr_adapter.zig` — dispatch function and per-protocol routers.
