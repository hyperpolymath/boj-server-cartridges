...
= ums-mcp / ffi — Zig FFI layer **(stub)**
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libums.so`.
| `ums_ffi.zig`     | C-ABI exports (15 exports, 6 inline tests, 360 lines).
|===
...
6 inline `test "..."` block(s) in `ums_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `ums_ffi.zig` — C-ABI signatures and guards.
