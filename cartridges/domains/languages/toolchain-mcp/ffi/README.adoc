...
= toolchain-mcp / ffi — Zig FFI layer **(stub)**
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libtoolchain.so`.
| `toolchain_mcp_ffi.zig`     | C-ABI exports (2 exports, 0 inline tests, 15 lines).
|===
...
0 inline `test "..."` block(s) in `toolchain_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `toolchain_mcp_ffi.zig` — C-ABI signatures and guards.
