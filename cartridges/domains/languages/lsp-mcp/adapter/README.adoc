...
= lsp-mcp / adapter — protocol bridge
:orientation: shallow
...
| `lsp_adapter.zig` | Protocol dispatch (720 lines). Tool catalogue matches `cartridge.json`.
...
. `lsp_adapter.zig` — dispatch function and per-protocol routers.
