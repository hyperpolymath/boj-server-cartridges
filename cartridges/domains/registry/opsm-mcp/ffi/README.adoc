...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libopsm.so`.
| `opsm_ffi.zig`     | C-ABI exports (9 exports, 4 inline tests, 174 lines).
|===
...
4 inline `test "..."` block(s) in `opsm_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `opsm_ffi.zig` — C-ABI signatures and guards.
