...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libpypi.so`.
| `pypi_mcp_ffi.zig`     | C-ABI exports (16 exports, 7 inline tests, 440 lines).
|===
...
7 inline `test "..."` block(s) in `pypi_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `pypi_mcp_ffi.zig` — C-ABI signatures and guards.
