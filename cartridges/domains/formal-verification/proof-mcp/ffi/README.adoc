...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libproof.so`.
| `proof_ffi.zig`     | C-ABI exports (15 exports, 8 inline tests, 318 lines).
|===
...
8 inline `test "..."` block(s) in `proof_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `proof_ffi.zig` — C-ABI signatures and guards.
