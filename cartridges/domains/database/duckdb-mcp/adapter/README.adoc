...
= duckdb-mcp / adapter — protocol bridge
:orientation: shallow
...
| `duckdb_mcp_adapter.zig` | Protocol dispatch (112 lines). Tool catalogue matches `cartridge.json`.
...
. `duckdb_mcp_adapter.zig` — dispatch function and per-protocol routers.
