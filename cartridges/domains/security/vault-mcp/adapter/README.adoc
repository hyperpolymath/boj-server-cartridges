...
= vault-mcp / adapter — protocol bridge
:orientation: shallow
...
| `vault_mcp_adapter.zig` | Protocol dispatch (134 lines). Tool catalogue matches `cartridge.json`.
...
. `vault_mcp_adapter.zig` — dispatch function and per-protocol routers.
