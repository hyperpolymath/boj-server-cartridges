...
= notion-mcp / adapter — protocol bridge
:orientation: shallow
...
| `notion_mcp_adapter.zig` | Protocol dispatch (144 lines). Tool catalogue matches `cartridge.json`.
...
. `notion_mcp_adapter.zig` — dispatch function and per-protocol routers.
