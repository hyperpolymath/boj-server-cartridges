...
# Integration tests for todoist-mcp cartridge.
set -euo pipefail
...
echo "=== todoist-mcp integration tests ==="
...
if command -v idris2 &>/dev/null; then
    cd "$CART_DIR/abi" && idris2 --check TodoistMcp.SafeRegistry 2>&1
    echo "  ABI: OK"
else
...
echo ""
echo "All tests passed for todoist-mcp!"
