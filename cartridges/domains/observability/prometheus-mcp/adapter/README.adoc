...
= prometheus-mcp / adapter — protocol bridge
:orientation: shallow
...
| `prometheus_adapter.zig` | Protocol dispatch (196 lines). Tool catalogue matches `cartridge.json`.
...
. `prometheus_adapter.zig` — dispatch function and per-protocol routers.
