...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libhetzner.so`.
| `hetzner_mcp_ffi.zig`     | C-ABI exports (17 exports, 8 inline tests, 445 lines).
|===
...
8 inline `test "..."` block(s) in `hetzner_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `hetzner_mcp_ffi.zig` — C-ABI signatures and guards.
