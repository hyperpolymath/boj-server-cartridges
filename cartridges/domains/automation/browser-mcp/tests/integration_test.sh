...
# Integration tests for browser-mcp cartridge.
set -euo pipefail
...
echo "=== browser-mcp integration tests ==="
...
if command -v idris2 &>/dev/null; then
    cd "$CART_DIR/abi" && idris2 --check BrowserMcp.SafeBrowser 2>&1
    echo "  ABI: OK"
else
...
echo ""
echo "All tests passed for browser-mcp!"
