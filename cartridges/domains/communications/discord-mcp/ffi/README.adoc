...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libdiscord.so`.
| `discord_mcp_ffi.zig`     | C-ABI exports (13 exports, 6 inline tests, 374 lines).
|===
...
6 inline `test "..."` block(s) in `discord_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `discord_mcp_ffi.zig` — C-ABI signatures and guards.
