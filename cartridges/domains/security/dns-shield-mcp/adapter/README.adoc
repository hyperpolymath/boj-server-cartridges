...
= dns-shield-mcp / adapter — protocol bridge
:orientation: shallow
...
| `dns_shield_adapter.zig` | Protocol dispatch (134 lines). Tool catalogue matches `cartridge.json`.
...
. `dns_shield_adapter.zig` — dispatch function and per-protocol routers.
