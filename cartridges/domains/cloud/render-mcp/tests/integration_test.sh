...
# Integration tests for render-mcp cartridge.
set -euo pipefail
...
echo "=== render-mcp integration tests ==="
...
if command -v idris2 &>/dev/null; then
    cd "$CART_DIR/abi" && idris2 --check RenderMcp.SafeCloud 2>&1
    echo "  ABI: OK"
else
...
echo ""
echo "All tests passed for render-mcp!"
