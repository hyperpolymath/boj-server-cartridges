...
# Integration tests for turso-mcp cartridge.
set -euo pipefail
...
echo "=== turso-mcp integration tests ==="
...
if command -v idris2 &>/dev/null; then
    cd "$CART_DIR/abi" && idris2 --check TursoMcp.SafeDatabase 2>&1
    echo "  ABI: OK"
else
...
echo ""
echo "All tests passed for turso-mcp!"
