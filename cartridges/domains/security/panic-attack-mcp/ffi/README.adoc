...
= panic-attack-mcp / ffi — Zig FFI layer **(stub)**
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libpanic-attack.so`.
| `panic_attack_ffi.zig`     | C-ABI exports (3 exports, 3 inline tests, 42 lines).
|===
...
3 inline `test "..."` block(s) in `panic_attack_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `panic_attack_ffi.zig` — C-ABI signatures and guards.
