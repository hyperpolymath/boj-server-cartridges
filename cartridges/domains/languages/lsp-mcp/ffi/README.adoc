...
| `build.zig`      | Zig build graph — produces `zig-out/lib/liblsp.so`.
| `lsp_ffi.zig`     | C-ABI exports (15 exports, 5 inline tests, 302 lines).
|===
...
5 inline `test "..."` block(s) in `lsp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `lsp_ffi.zig` — C-ABI signatures and guards.
