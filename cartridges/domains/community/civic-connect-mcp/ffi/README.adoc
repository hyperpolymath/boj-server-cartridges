...
= civic-connect-mcp / ffi — Zig FFI layer **(stub)**
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libcivic-connect.so`.
| `civic_connect_ffi.zig`     | C-ABI exports (3 exports, 3 inline tests, 37 lines).
|===
...
3 inline `test "..."` block(s) in `civic_connect_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `civic_connect_ffi.zig` — C-ABI signatures and guards.
