...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libcomms.so`.
| `comms_ffi.zig`     | C-ABI exports (22 exports, 12 inline tests, 543 lines).
|===
...
12 inline `test "..."` block(s) in `comms_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `comms_ffi.zig` — C-ABI signatures and guards.
