...
= hetzner-mcp / adapter — protocol bridge
:orientation: shallow
...
| `hetzner_adapter.zig` | Protocol dispatch (244 lines). Tool catalogue matches `cartridge.json`.
...
. `hetzner_adapter.zig` — dispatch function and per-protocol routers.
