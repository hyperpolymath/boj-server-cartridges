...
= railway-mcp / adapter — protocol bridge
:orientation: shallow
...
| `railway_adapter.zig` | Protocol dispatch (240 lines). Tool catalogue matches `cartridge.json`.
...
. `railway_adapter.zig` — dispatch function and per-protocol routers.
