...
= dns-shield-mcp / ffi — Zig FFI layer
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libdns-shield.so`.
| `dns_shield_ffi.zig`     | C-ABI exports (6 exports, 4 inline tests, 160 lines).
|===
...
4 inline `test "..."` block(s) in `dns_shield_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `dns_shield_ffi.zig` — C-ABI signatures and guards.
