...
//
// k9iser-mcp/adapter/k9iser_adapter.zig
//
...
// Idris2 K9iserMcp.SafeK9iser.exposureSatisfied contract) BEFORE dispatch.
...
// ── Transaction gate (mirrors Idris2 SafeK9iser.exposureSatisfied) ──────
//
// Encoding matches K9iserMcp.SafeK9iser: 0=Public 1=Authenticated 2=Internal.
const Exposure = enum(u8) { public = 0, authenticated = 1, internal = 2 };

// k9iser-mcp cartridge.json: auth.method = "none" → requiredExposure = Public.
    ...
    switch (proto) {
        ...
        .graphql => {
            const tools = [_][]const u8{
                "k9_load_manifest", "k9_generate", "k9_validate", "k9_apply", "k9_clean",
            };
...
test "classify routes each protocol to one surface" {
    ...
    try std.testing.expectEqual(Protocol.grpc, classify("/grpc/K9/k9_generate"));
    try std.testing.expectEqual(Protocol.unknown, classify("/nope"));
}

test "toolFor extracts across protocols" {
    try std.testing.expectEqualStrings("k9_generate", toolFor(.grpc, "/grpc/K9/k9_generate", "").?);
    try std.testing.expectEqualStrings("k9_apply", toolFor(.rest, "/invoke?tool=k9_apply", "").?);
    try std.testing.expectEqualStrings("k9_validate", toolFor(.sse, "/sse", "{\"tool\":\"k9_validate\"}").?);
    try std.testing.expectEqualStrings("k9_clean", toolFor(.graphql, "/graphql", "{query: invoke(tool:\"k9_clean\")}").?);
    try std.testing.expect(toolFor(.rest, "/invoke", "{}") == null);
}

test "dispatch funnels into the one Zig ABI" {
    var out: [256]u8 = undefined;
    const d = dispatch("k9_generate", "{}", &out);
...
// K9iserMcp.SafeK9iser.exposureSatisfied exactly.
