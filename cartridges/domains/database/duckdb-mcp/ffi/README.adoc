...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libduckdb.so`.
| `duckdb_mcp_ffi.zig`     | C-ABI exports (13 exports, 7 inline tests, 385 lines).
|===
...
7 inline `test "..."` block(s) in `duckdb_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `duckdb_mcp_ffi.zig` — C-ABI signatures and guards.
