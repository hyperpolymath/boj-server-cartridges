...
= secrets-mcp / adapter — protocol bridge
:orientation: shallow
...
| `secrets_adapter.zig` | Protocol dispatch (134 lines). Tool catalogue matches `cartridge.json`.
...
. `secrets_adapter.zig` — dispatch function and per-protocol routers.
