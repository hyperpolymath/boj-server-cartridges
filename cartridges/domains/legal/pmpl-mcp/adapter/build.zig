...
pub fn build(b: *std.Build) void {
    ...
    const ffi_mod = b.createModule(.{
        .root_source_file = b.path("../ffi/pmpl_ffi.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const adapter_mod = b.createModule(.{
        .root_source_file = b.path("pmpl_adapter.zig"),
        .target = target,
        .optimize = optimize,
    });
    adapter_mod.addImport("pmpl_ffi", ffi_mod);
    ...
    const run_step = b.step("run", "Run the pmpl-mcp adapter");
    ...
    const test_step = b.step("test", "Run pmpl-mcp adapter tests");
