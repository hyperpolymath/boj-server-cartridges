...
// Tools: kategoria_classify, kategoria_get_levels, kategoria_eval_challenge

const std = @import("std");
const ffi = @import("kategoria_ffi");

const REST_PORT: u16 = 9127;
const GRPC_PORT: u16 = 9128;
const GQL_PORT:  u16 = 9129;
...
fn statusJson(buf: []u8) []u8 {
    const n = std.fmt.bufPrint(buf, "{{\"success\":true,\"state\":\"ready\",\"service\":\"kategoria-mcp\"}}", .{}) catch return buf[0..0];
    return n;
}
...
fn dispatch(tool: []const u8, body: []const u8, resp: []u8) Response {
    _ = body;
    if (std.mem.eql(u8, tool, "kategoria_classify")) return .{ .status = 200, .body = okJson(resp, "kategoria_classify forwarded") };
    if (std.mem.eql(u8, tool, "kategoria_get_levels")) return .{ .status = 200, .body = okJson(resp, "kategoria_get_levels forwarded") };
    if (std.mem.eql(u8, tool, "kategoria_eval_challenge")) return .{ .status = 200, .body = okJson(resp, "kategoria_eval_challenge forwarded") };
...
fn dispatchGrpc(path: []const u8, body: []const u8, resp: []u8) Response {
    const prefix = "/Kategoriaservice/";
    if (!std.mem.startsWith(u8, path, prefix))
        return .{ .status = 404, .body = errJson(resp, "Not a recognized gRPC path") };
    const method = path[prefix.len..];
    const tool = blk: {
        if (std.mem.eql(u8, method, "kategoria_classify")) break :blk "kategoria_classify";
        if (std.mem.eql(u8, method, "kategoria_get_levels")) break :blk "kategoria_get_levels";
        if (std.mem.eql(u8, method, "kategoria_eval_challenge")) break :blk "kategoria_eval_challenge";
...
fn dispatchGraphql(body: []const u8, resp: []u8) Response {
    ...
    if (std.mem.indexOf(u8, body, "classify") != null) return dispatch("kategoria_classify", body, resp);
    if (std.mem.indexOf(u8, body, "get_levels") != null) return dispatch("kategoria_get_levels", body, resp);
    if (std.mem.indexOf(u8, body, "eval_challenge") != null) return dispatch("kategoria_eval_challenge", body, resp);
