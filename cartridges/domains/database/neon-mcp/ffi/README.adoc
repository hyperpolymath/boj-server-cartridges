...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libneon.so`.
| `neon_mcp_ffi.zig`     | C-ABI exports (10 exports, 5 inline tests, 293 lines).
|===
...
5 inline `test "..."` block(s) in `neon_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `neon_mcp_ffi.zig` — C-ABI signatures and guards.
