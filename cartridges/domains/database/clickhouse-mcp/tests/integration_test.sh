...
# Integration tests for clickhouse-mcp cartridge.
set -euo pipefail
...
echo "=== clickhouse-mcp integration tests ==="
...
if command -v idris2 &>/dev/null; then
    cd "$CART_DIR/abi" && idris2 --check ClickhouseMcp.SafeDatabase 2>&1
    echo "  ABI: OK"
else
...
echo ""
echo "All tests passed for clickhouse-mcp!"
