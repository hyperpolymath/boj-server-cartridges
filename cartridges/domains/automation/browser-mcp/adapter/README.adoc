...
= browser-mcp / adapter — protocol bridge
:orientation: shallow
...
| `browser_mcp_adapter.zig` | Protocol dispatch (154 lines). Tool catalogue matches `cartridge.json`.
...
. `browser_mcp_adapter.zig` — dispatch function and per-protocol routers.
