...
pub fn build(b: *std.Build) void {
    ...
    const ffi_mod = b.createModule(.{
        .root_source_file = b.path("../ffi/cloud_ffi.zig"),
        .target = target,
        .optimize = optimize,
    });

    const adapter_mod = b.createModule(.{
        .root_source_file = b.path("cloud_adapter.zig"),
        .target = target,
        .optimize = optimize,
    });
    adapter_mod.addImport("cloud_ffi", ffi_mod);
    ...
    const run_step = b.step("run", "Run the cloud-mcp adapter");
    ...
    const test_step = b.step("test", "Run cloud-mcp adapter tests");
