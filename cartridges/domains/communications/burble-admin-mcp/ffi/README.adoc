...
= burble-admin-mcp / ffi — Zig FFI layer **(stub)**
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libburble-admin.so`.
| `burble_admin_ffi.zig`     | C-ABI exports (3 exports, 3 inline tests, 97 lines).
|===
...
3 inline `test "..."` block(s) in `burble_admin_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `burble_admin_ffi.zig` — C-ABI signatures and guards.
