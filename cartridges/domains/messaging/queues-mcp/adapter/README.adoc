...
= queues-mcp / adapter — protocol bridge
:orientation: shallow
...
| `queues_adapter.zig` | Protocol dispatch (139 lines). Tool catalogue matches `cartridge.json`.
...
. `queues_adapter.zig` — dispatch function and per-protocol routers.
