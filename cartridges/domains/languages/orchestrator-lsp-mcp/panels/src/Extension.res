...
let resolveAdapterDir = (): string => {
  ...
  | _ =>
    ...
    | Some(d) => d
    | None => "cartridges/domains/languages/orchestrator-lsp-mcp/adapter"
    }
  }
}
...
let resolveWorkspaceRoot = (): string => {
  ...
  | Some(folders) if Array.length(folders) > 0 =>
    let uri = folders->Belt.Array.getExn(0)->VscodeApi.WorkspaceFolder.uri
