...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libsentry.so`.
| `sentry_mcp_ffi.zig`     | C-ABI exports (14 exports, 5 inline tests, 322 lines).
|===
...
5 inline `test "..."` block(s) in `sentry_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `sentry_mcp_ffi.zig` — C-ABI signatures and guards.
