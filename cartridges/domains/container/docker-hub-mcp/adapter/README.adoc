...
= docker-hub-mcp / adapter — protocol bridge
:orientation: shallow
...
| `docker_hub_adapter.zig` | Protocol dispatch (228 lines). Tool catalogue matches `cartridge.json`.
...
. `docker_hub_adapter.zig` — dispatch function and per-protocol routers.
