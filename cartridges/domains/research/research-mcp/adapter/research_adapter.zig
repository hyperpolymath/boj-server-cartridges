...
//
// research-mcp/adapter/research_adapter.zig
//
// Three-protocol BoJ adapter: REST (port 9283), gRPC-compat (port 9284),
// GraphQL (port 9285).
// Replaces the banned zig adapter (research_adapter.v).

const std = @import("std");
const ffi = @import("research_ffi");

const REST_PORT: u16 = 9283;
const GRPC_PORT: u16 = 9284;
const GQL_PORT:  u16 = 9285;
...
fn dispatch(tool: []const u8, _body: []const u8, resp: []u8) Response {
    _ = _body;
    if (std.mem.eql(u8, tool, "research_authenticate")) {
        return .{ .status = 200, .body = okJson(resp, "research_authenticate forwarded") };
    }
    if (std.mem.eql(u8, tool, "research_search")) {
        return .{ .status = 200, .body = okJson(resp, "research_search forwarded") };
    }
    if (std.mem.eql(u8, tool, "research_get_paper")) {
        return .{ .status = 200, .body = okJson(resp, "research_get_paper forwarded") };
    }
    if (std.mem.eql(u8, tool, "research_list_providers")) {
        return .{ .status = 200, .body = okJson(resp, "research_list_providers forwarded") };
    }
...
fn dispatchGraphql(body: []const u8, resp: []u8) Response {
    if (std.mem.indexOf(u8, body, "research_authenticate") != null)
        return dispatch("research_authenticate", body, resp);
    if (std.mem.indexOf(u8, body, "research_search") != null)
        return dispatch("research_search", body, resp);
    if (std.mem.indexOf(u8, body, "research_get_paper") != null)
        return dispatch("research_get_paper", body, resp);
    if (std.mem.indexOf(u8, body, "research_list_providers") != null)
        return dispatch("research_list_providers", body, resp);
