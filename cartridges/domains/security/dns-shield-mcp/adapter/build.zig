...
//
// dns-shield-mcp/adapter/build.zig
...
pub fn build(b: *std.Build) void {
    ...
    const ffi_mod = b.createModule(.{
        .root_source_file = b.path("../ffi/dns_shield_ffi.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const adapter_mod = b.createModule(.{
        .root_source_file = b.path("dns_shield_adapter.zig"),
        .target = target,
        .optimize = optimize,
    });
    adapter_mod.addImport("dns_shield_ffi", ffi_mod);

    const adapter = b.addExecutable(.{
        .name = "dns_shield_adapter",
        .root_module = adapter_mod,
    });
    b.installArtifact(adapter);

    const run_step = b.step("run", "Run the dns-shield-mcp adapter");
    ...
    const test_step = b.step("test", "Run dns-shield-mcp adapter tests");
