...
// Tools: vordr_scan, vordr_set_baseline, vordr_alerts, vordr_compare

const std = @import("std");
const ffi = @import("vordr_ffi");

const REST_PORT: u16 = 9136;
const GRPC_PORT: u16 = 9137;
const GQL_PORT:  u16 = 9138;
...
fn statusJson(buf: []u8) []u8 {
    const n = std.fmt.bufPrint(buf, "{{\"success\":true,\"state\":\"ready\",\"service\":\"vordr-mcp\"}}", .{}) catch return buf[0..0];
    return n;
}
...
fn dispatch(tool: []const u8, body: []const u8, resp: []u8) Response {
    _ = body;
    if (std.mem.eql(u8, tool, "vordr_scan")) return .{ .status = 200, .body = okJson(resp, "vordr_scan forwarded") };
    if (std.mem.eql(u8, tool, "vordr_set_baseline")) return .{ .status = 200, .body = okJson(resp, "vordr_set_baseline forwarded") };
    if (std.mem.eql(u8, tool, "vordr_alerts")) return .{ .status = 200, .body = okJson(resp, "vordr_alerts forwarded") };
    if (std.mem.eql(u8, tool, "vordr_compare")) return .{ .status = 200, .body = okJson(resp, "vordr_compare forwarded") };
...
fn dispatchGrpc(path: []const u8, body: []const u8, resp: []u8) Response {
    const prefix = "/Vordrservice/";
    if (!std.mem.startsWith(u8, path, prefix))
        return .{ .status = 404, .body = errJson(resp, "Not a recognized gRPC path") };
    const method = path[prefix.len..];
    const tool = blk: {
        if (std.mem.eql(u8, method, "vordr_scan")) break :blk "vordr_scan";
        if (std.mem.eql(u8, method, "vordr_set_baseline")) break :blk "vordr_set_baseline";
        if (std.mem.eql(u8, method, "vordr_alerts")) break :blk "vordr_alerts";
        if (std.mem.eql(u8, method, "vordr_compare")) break :blk "vordr_compare";
...
fn dispatchGraphql(body: []const u8, resp: []u8) Response {
    ...
    if (std.mem.indexOf(u8, body, "scan") != null) return dispatch("vordr_scan", body, resp);
    if (std.mem.indexOf(u8, body, "set_baseline") != null) return dispatch("vordr_set_baseline", body, resp);
    if (std.mem.indexOf(u8, body, "alerts") != null) return dispatch("vordr_alerts", body, resp);
    if (std.mem.indexOf(u8, body, "compare") != null) return dispatch("vordr_compare", body, resp);
