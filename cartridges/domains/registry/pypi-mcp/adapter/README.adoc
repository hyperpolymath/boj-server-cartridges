...
= pypi-mcp / adapter — protocol bridge
:orientation: shallow
...
| `pypi_adapter.zig` | Protocol dispatch (208 lines). Tool catalogue matches `cartridge.json`.
...
. `pypi_adapter.zig` — dispatch function and per-protocol routers.
