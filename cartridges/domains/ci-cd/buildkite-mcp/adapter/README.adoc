...
= buildkite-mcp / adapter — protocol bridge
:orientation: shallow
...
| `buildkite_adapter.zig` | Protocol dispatch (204 lines). Tool catalogue matches `cartridge.json`.
...
. `buildkite_adapter.zig` — dispatch function and per-protocol routers.
