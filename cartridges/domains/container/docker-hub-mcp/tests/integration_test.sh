...
# Integration tests for docker-hub-mcp cartridge.
set -euo pipefail
...
echo "=== docker-hub-mcp integration tests ==="
...
if command -v idris2 &>/dev/null; then
    cd "$CART_DIR/abi" && idris2 --check DockerHubMcp.SafeContainer 2>&1
    echo "  ABI: OK"
else
...
echo ""
echo "All tests passed for docker-hub-mcp!"
