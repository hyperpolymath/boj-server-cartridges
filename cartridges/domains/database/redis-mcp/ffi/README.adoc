...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libredis.so`.
| `redis_mcp_ffi.zig`     | C-ABI exports (12 exports, 6 inline tests, 371 lines).
|===
...
6 inline `test "..."` block(s) in `redis_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `redis_mcp_ffi.zig` — C-ABI signatures and guards.
