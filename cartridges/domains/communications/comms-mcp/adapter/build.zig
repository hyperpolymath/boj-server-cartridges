...
pub fn build(b: *std.Build) void {
    ...
    const ffi_mod = b.createModule(.{
        .root_source_file = b.path("../ffi/comms_ffi.zig"),
        .target = target,
        .optimize = optimize,
    });

    const adapter_mod = b.createModule(.{
        .root_source_file = b.path("comms_adapter.zig"),
        .target = target,
        .optimize = optimize,
    });
    adapter_mod.addImport("comms_ffi", ffi_mod);
    ...
    const run_step = b.step("run", "Run the comms-mcp adapter");
    ...
    const test_step = b.step("test", "Run comms-mcp adapter tests");
