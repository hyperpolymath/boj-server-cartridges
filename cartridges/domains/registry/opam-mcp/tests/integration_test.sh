...
# Integration tests for opam-mcp cartridge.
set -euo pipefail
...
echo "=== opam-mcp integration tests ==="
...
if command -v idris2 &>/dev/null; then
    cd "$CART_DIR/abi" && idris2 --check OpamMcp.SafeRegistry 2>&1
    echo "  ABI: OK"
else
...
echo ""
echo "All tests passed for opam-mcp!"
