...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libmatrix.so`.
| `matrix_mcp_ffi.zig`     | C-ABI exports (15 exports, 7 inline tests, 391 lines).
|===
...
7 inline `test "..."` block(s) in `matrix_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `matrix_mcp_ffi.zig` — C-ABI signatures and guards.
