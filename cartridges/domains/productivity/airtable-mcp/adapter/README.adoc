...
= airtable-mcp / adapter — protocol bridge
:orientation: shallow
...
| `airtable_adapter.zig` | Protocol dispatch (204 lines). Tool catalogue matches `cartridge.json`.
...
. `airtable_adapter.zig` — dispatch function and per-protocol routers.
