...
= rokur-mcp / adapter — protocol bridge
:orientation: shallow
...
| `rokur_mcp_adapter.zig` | Protocol dispatch (129 lines). Tool catalogue matches `cartridge.json`.
...
. `rokur_mcp_adapter.zig` — dispatch function and per-protocol routers.
