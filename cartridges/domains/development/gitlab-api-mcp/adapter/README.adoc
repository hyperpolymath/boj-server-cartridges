...
= gitlab-api-mcp / adapter — protocol bridge
:orientation: shallow
...
| `gitlab_api_mcp_adapter.zig` | Protocol dispatch (144 lines). Tool catalogue matches `cartridge.json`.
...
. `gitlab_api_mcp_adapter.zig` — dispatch function and per-protocol routers.
