...
= google-docs-mcp / adapter — protocol bridge
:orientation: shallow
...
| `google_docs_adapter.zig` | Protocol dispatch (204 lines). Tool catalogue matches `cartridge.json`.
...
. `google_docs_adapter.zig` — dispatch function and per-protocol routers.
