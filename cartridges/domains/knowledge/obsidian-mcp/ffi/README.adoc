...
= obsidian-mcp / ffi — Zig FFI layer
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libobsidian.so`.
| `obsidian_mcp_ffi.zig`     | C-ABI exports (15 exports, 6 inline tests, 373 lines).
|===
...
6 inline `test "..."` block(s) in `obsidian_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `obsidian_mcp_ffi.zig` — C-ABI signatures and guards.
