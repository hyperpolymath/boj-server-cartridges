...
# Integration tests for redis-mcp cartridge.
set -euo pipefail
...
echo "=== redis-mcp integration tests ==="
...
if command -v idris2 &>/dev/null; then
    cd "$CART_DIR/abi" && idris2 --check RedisMcp.SafeDatabase 2>&1
    echo "  ABI: OK"
else
...
echo ""
echo "All tests passed for redis-mcp!"
