...
= google-sheets-mcp / ffi — Zig FFI layer
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libgoogle-sheets.so`.
| `google_sheets_mcp_ffi.zig`     | C-ABI exports (15 exports, 5 inline tests, 333 lines).
|===
...
5 inline `test "..."` block(s) in `google_sheets_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `google_sheets_mcp_ffi.zig` — C-ABI signatures and guards.
