...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libbsp.so`.
| `bsp_ffi.zig`     | C-ABI exports (20 exports, 5 inline tests, 372 lines).
|===
...
5 inline `test "..."` block(s) in `bsp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `bsp_ffi.zig` — C-ABI signatures and guards.
