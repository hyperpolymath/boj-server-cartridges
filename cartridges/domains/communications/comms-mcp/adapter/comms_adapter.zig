...
//
// comms-mcp/adapter/comms_adapter.zig
//
// Three-protocol BoJ adapter: REST (port 9217), gRPC-compat (port 9218),
// GraphQL (port 9219).
// Replaces the banned zig adapter (comms_adapter.v).

const std = @import("std");
const ffi = @import("comms_ffi");

const REST_PORT: u16 = 9217;
const GRPC_PORT: u16 = 9218;
const GQL_PORT:  u16 = 9219;
...
fn dispatch(tool: []const u8, _body: []const u8, resp: []u8) Response {
    _ = _body;
    if (std.mem.eql(u8, tool, "comms_authenticate")) {
        return .{ .status = 200, .body = okJson(resp, "comms_authenticate forwarded") };
    }
    if (std.mem.eql(u8, tool, "comms_logout")) {
        return .{ .status = 200, .body = okJson(resp, "comms_logout forwarded") };
    }
    if (std.mem.eql(u8, tool, "comms_execute")) {
        return .{ .status = 200, .body = okJson(resp, "comms_execute forwarded") };
    }
    if (std.mem.eql(u8, tool, "comms_state")) {
        return .{ .status = 200, .body = okJson(resp, "comms_state forwarded") };
    }
...
fn dispatchGraphql(body: []const u8, resp: []u8) Response {
    if (std.mem.indexOf(u8, body, "comms_authenticate") != null)
        return dispatch("comms_authenticate", body, resp);
    if (std.mem.indexOf(u8, body, "comms_logout") != null)
        return dispatch("comms_logout", body, resp);
    if (std.mem.indexOf(u8, body, "comms_execute") != null)
        return dispatch("comms_execute", body, resp);
    if (std.mem.indexOf(u8, body, "comms_state") != null)
        return dispatch("comms_state", body, resp);
