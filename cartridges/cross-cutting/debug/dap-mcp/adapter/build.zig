...
pub fn build(b: *std.Build) void {
    ...
    adapter.root_module.addImport("dap_ffi", ffi_mod);
    b.installArtifact(adapter);

    const run_cmd  = b.addRunArtifact(adapter);
    const run_step = b.step("run", "Run dap-adapter (REST :9019, gRPC :9020, GraphQL :9021)");
    ...
    tests.root_module.addImport("dap_ffi", ffi_mod);
