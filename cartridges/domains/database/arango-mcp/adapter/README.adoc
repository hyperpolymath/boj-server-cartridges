...
= arango-mcp / adapter — protocol bridge
:orientation: shallow
...
| `arango_mcp_adapter.zig` | Protocol dispatch (118 lines). Tool catalogue matches `cartridge.json`.
...
. `arango_mcp_adapter.zig` — dispatch function and per-protocol routers.
