...
# Integration tests for neo4j-mcp cartridge.
set -euo pipefail
...
echo "=== neo4j-mcp integration tests ==="
...
if command -v idris2 &>/dev/null; then
    cd "$CART_DIR/abi" && idris2 --check Neo4jMcp.SafeDatabase 2>&1
    echo "  ABI: OK"
else
...
echo ""
echo "All tests passed for neo4j-mcp!"
