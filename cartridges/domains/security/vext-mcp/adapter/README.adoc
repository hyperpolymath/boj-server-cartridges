...
= vext-mcp / adapter — protocol bridge
:orientation: shallow
...
| `vext_adapter.zig` | Protocol dispatch (118 lines). Tool catalogue matches `cartridge.json`.
...
. `vext_adapter.zig` — dispatch function and per-protocol routers.
