...
= supabase-mcp / adapter — protocol bridge
:orientation: shallow
...
| `supabase_mcp_adapter.zig` | Protocol dispatch (112 lines). Tool catalogue matches `cartridge.json`.
...
. `supabase_mcp_adapter.zig` — dispatch function and per-protocol routers.
