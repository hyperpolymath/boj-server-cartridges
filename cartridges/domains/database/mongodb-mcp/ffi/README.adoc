...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libmongodb.so`.
| `mongodb_mcp_ffi.zig`     | C-ABI exports (11 exports, 6 inline tests, 353 lines).
|===
...
6 inline `test "..."` block(s) in `mongodb_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `mongodb_mcp_ffi.zig` — C-ABI signatures and guards.
