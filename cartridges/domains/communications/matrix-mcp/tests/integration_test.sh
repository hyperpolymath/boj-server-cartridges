...
# Integration tests for matrix-mcp cartridge.
set -euo pipefail
...
echo "=== matrix-mcp integration tests ==="
...
if command -v idris2 &>/dev/null; then
    cd "$CART_DIR/abi" && idris2 --check MatrixMcp.SafeComms 2>&1
    echo "  ABI: OK"
else
...
echo ""
echo "All tests passed for matrix-mcp!"
