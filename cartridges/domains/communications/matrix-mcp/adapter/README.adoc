...
= matrix-mcp / adapter — protocol bridge
:orientation: shallow
...
| `matrix_mcp_adapter.zig` | Protocol dispatch (112 lines). Tool catalogue matches `cartridge.json`.
...
. `matrix_mcp_adapter.zig` — dispatch function and per-protocol routers.
