...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libfly.so`.
| `fly_mcp_ffi.zig`     | C-ABI exports (14 exports, 6 inline tests, 348 lines).
|===
...
6 inline `test "..."` block(s) in `fly_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `fly_mcp_ffi.zig` — C-ABI signatures and guards.
