...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libgcp.so`.
| `gcp_mcp_ffi.zig`     | C-ABI exports (14 exports, 7 inline tests, 431 lines).
|===
...
7 inline `test "..."` block(s) in `gcp_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `gcp_mcp_ffi.zig` — C-ABI signatures and guards.
