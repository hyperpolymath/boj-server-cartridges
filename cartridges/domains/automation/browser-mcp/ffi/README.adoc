...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libbrowser.so`.
| `browser_mcp_ffi.zig`     | C-ABI exports (17 exports, 8 inline tests, 574 lines).
|===
...
8 inline `test "..."` block(s) in `browser_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `browser_mcp_ffi.zig` — C-ABI signatures and guards.
