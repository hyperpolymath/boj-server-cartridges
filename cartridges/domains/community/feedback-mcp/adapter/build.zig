...
//
// feedback-mcp/adapter/build.zig
...
pub fn build(b: *std.Build) void {
    ...
    const ffi_mod = b.createModule(.{
        .root_source_file = b.path("../ffi/feedback_ffi.zig"),
        .target = target,
        .optimize = optimize,
    });

    const adapter_mod = b.createModule(.{
        .root_source_file = b.path("feedback_adapter.zig"),
        .target = target,
        .optimize = optimize,
    });
    adapter_mod.addImport("feedback_ffi", ffi_mod);
    ...
    const run_step = b.step("run", "Run the feedback-mcp adapter");
    ...
    const test_step = b.step("test", "Run feedback-mcp adapter tests");
