...
# Integration tests for supabase-mcp cartridge.
set -euo pipefail
...
echo "=== supabase-mcp integration tests ==="
...
if command -v idris2 &>/dev/null; then
    cd "$CART_DIR/abi" && idris2 --check SupabaseMcp.SafeDatabase 2>&1
    echo "  ABI: OK"
else
...
echo ""
echo "All tests passed for supabase-mcp!"
