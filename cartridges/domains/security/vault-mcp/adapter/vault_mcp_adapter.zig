...
//
// vault-mcp/adapter/vault_mcp_adapter.zig
//
// Three-protocol BoJ adapter: REST (port 9298), gRPC-compat (port 9299),
// GraphQL (port 9300).
// Replaces the banned zig adapter (vault_mcp_adapter.v).

const std = @import("std");
const ffi = @import("vault_mcp_ffi");

const REST_PORT: u16 = 9298;
const GRPC_PORT: u16 = 9299;
const GQL_PORT:  u16 = 9300;
...
fn dispatch(tool: []const u8, _body: []const u8, resp: []u8) Response {
    _ = _body;
    if (std.mem.eql(u8, tool, "vault_execute")) {
        return .{ .status = 200, .body = okJson(resp, "vault_execute forwarded") };
    }
    if (std.mem.eql(u8, tool, "vault_list")) {
        return .{ .status = 200, .body = okJson(resp, "vault_list forwarded") };
    }
    if (std.mem.eql(u8, tool, "vault_status")) {
        return .{ .status = 200, .body = okJson(resp, "vault_status forwarded") };
    }
    if (std.mem.eql(u8, tool, "vault_verify")) {
        return .{ .status = 200, .body = okJson(resp, "vault_verify forwarded") };
    }
    if (std.mem.eql(u8, tool, "vault_rotate")) {
        return .{ .status = 200, .body = okJson(resp, "vault_rotate forwarded") };
    }
...
fn dispatchGraphql(body: []const u8, resp: []u8) Response {
    if (std.mem.indexOf(u8, body, "vault_execute") != null)
        return dispatch("vault_execute", body, resp);
    if (std.mem.indexOf(u8, body, "vault_list") != null)
        return dispatch("vault_list", body, resp);
    if (std.mem.indexOf(u8, body, "vault_status") != null)
        return dispatch("vault_status", body, resp);
    if (std.mem.indexOf(u8, body, "vault_verify") != null)
        return dispatch("vault_verify", body, resp);
    if (std.mem.indexOf(u8, body, "vault_rotate") != null)
        return dispatch("vault_rotate", body, resp);
