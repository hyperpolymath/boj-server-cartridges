...
//
// Comms-MCP Cartridge — Zig FFI build configuration (Zig 0.15+).
...
pub fn build(b: *std.Build) void {
    ...
    const comms_mod = b.addModule("comms_ffi", .{
        .root_source_file = b.path("comms_ffi.zig"),
        .target = target,
        .optimize = optimize,
    });

    comms_mod.addImport("cartridge_shim", shim_mod);

    // ── Tests ────────────────────────────────────────────────────────
    const comms_tests = b.addTest(.{
        .root_module = comms_mod,
    });

    const run_tests = b.addRunArtifact(comms_tests);

    const test_step = b.step("test", "Run comms-mcp FFI tests");
