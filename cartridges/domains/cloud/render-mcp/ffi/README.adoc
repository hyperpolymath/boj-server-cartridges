...
| `build.zig`      | Zig build graph — produces `zig-out/lib/librender.so`.
| `render_mcp_ffi.zig`     | C-ABI exports (18 exports, 7 inline tests, 401 lines).
|===
...
7 inline `test "..."` block(s) in `render_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `render_mcp_ffi.zig` — C-ABI signatures and guards.
