...
pub fn build(b: *std.Build) void {
    ...
    const ffi_mod = b.createModule(.{
        .root_source_file = b.path("../ffi/container_ffi.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const adapter_mod = b.createModule(.{
        .root_source_file = b.path("container_adapter.zig"),
        .target = target,
        .optimize = optimize,
    });
    adapter_mod.addImport("container_ffi", ffi_mod);
    ...
    const run_step = b.step("run", "Run the container-mcp adapter");
    ...
    const test_step = b.step("test", "Run container-mcp adapter tests");
