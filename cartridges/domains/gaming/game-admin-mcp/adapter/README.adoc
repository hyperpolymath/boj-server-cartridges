...
= game-admin-mcp / adapter — protocol bridge
:orientation: shallow
...
| `game_admin_adapter.zig` | Protocol dispatch (144 lines). Tool catalogue matches `cartridge.json`.
...
. `game_admin_adapter.zig` — dispatch function and per-protocol routers.
