...
# Integration tests for google-docs-mcp cartridge.
set -euo pipefail
...
echo "=== google-docs-mcp integration tests ==="
...
if command -v idris2 &>/dev/null; then
    cd "$CART_DIR/abi" && idris2 --check GoogleDocsMcp.SafeRegistry 2>&1
    echo "  ABI: OK"
else
...
echo ""
echo "All tests passed for google-docs-mcp!"
