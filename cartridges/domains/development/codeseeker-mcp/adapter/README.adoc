...
= codeseeker-mcp / adapter — protocol bridge
:orientation: shallow
...
| `codeseeker_adapter.zig` | Protocol dispatch (134 lines). Tool catalogue matches `cartridge.json`.
...
. `codeseeker_adapter.zig` — dispatch function and per-protocol routers.
