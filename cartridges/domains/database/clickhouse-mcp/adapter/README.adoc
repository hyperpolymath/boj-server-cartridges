...
= clickhouse-mcp / adapter — protocol bridge
:orientation: shallow
...
| `clickhouse_mcp_adapter.zig` | Protocol dispatch (112 lines). Tool catalogue matches `cartridge.json`.
...
. `clickhouse_mcp_adapter.zig` — dispatch function and per-protocol routers.
