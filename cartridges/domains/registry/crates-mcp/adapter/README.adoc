...
= crates-mcp / adapter — protocol bridge
:orientation: shallow
...
| `crates_adapter.zig` | Protocol dispatch (216 lines). Tool catalogue matches `cartridge.json`.
...
. `crates_adapter.zig` — dispatch function and per-protocol routers.
