...
= circleci-mcp / adapter — protocol bridge
:orientation: shallow
...
| `circleci_adapter.zig` | Protocol dispatch (200 lines). Tool catalogue matches `cartridge.json`.
...
. `circleci_adapter.zig` — dispatch function and per-protocol routers.
