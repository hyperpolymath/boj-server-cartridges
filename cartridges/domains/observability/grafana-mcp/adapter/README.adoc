...
= grafana-mcp / adapter — protocol bridge
:orientation: shallow
...
| `grafana_adapter.zig` | Protocol dispatch (204 lines). Tool catalogue matches `cartridge.json`.
...
. `grafana_adapter.zig` — dispatch function and per-protocol routers.
