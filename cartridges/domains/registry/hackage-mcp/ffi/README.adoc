...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libhackage.so`.
| `hackage_mcp_ffi.zig`     | C-ABI exports (16 exports, 7 inline tests, 405 lines).
|===
...
7 inline `test "..."` block(s) in `hackage_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `hackage_mcp_ffi.zig` — C-ABI signatures and guards.
