...
pub fn build(b: *std.Build) void {
    ...
    const ffi_mod = b.createModule(.{
        .root_source_file = b.path("../ffi/laminar_ffi.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const adapter_mod = b.createModule(.{
        .root_source_file = b.path("laminar_adapter.zig"),
        .target = target,
        .optimize = optimize,
    });
    adapter_mod.addImport("laminar_ffi", ffi_mod);

    const adapter = b.addExecutable(.{
        .name = "laminar_adapter",
        .root_module = adapter_mod,
    });
    b.installArtifact(adapter);

    const run_step = b.step("run", "Run the laminar-mcp adapter");
    ...
    const test_step = b.step("test", "Run laminar-mcp adapter tests");
