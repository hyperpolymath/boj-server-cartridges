...
= telegram-mcp / adapter — protocol bridge
:orientation: shallow
...
| `telegram_mcp_adapter.zig` | Protocol dispatch (109 lines). Tool catalogue matches `cartridge.json`.
...
. `telegram_mcp_adapter.zig` — dispatch function and per-protocol routers.
