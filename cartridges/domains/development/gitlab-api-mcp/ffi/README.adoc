...
= gitlab-api-mcp / ffi — Zig FFI layer
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libgitlab-api.so`.
| `gitlab_api_mcp_ffi.zig`     | C-ABI exports (15 exports, 8 inline tests, 850 lines).
|===
...
8 inline `test "..."` block(s) in `gitlab_api_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `gitlab_api_mcp_ffi.zig` — C-ABI signatures and guards.
