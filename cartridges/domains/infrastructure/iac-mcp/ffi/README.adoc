...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libiac.so`.
| `iac_ffi.zig`     | C-ABI exports (12 exports, 7 inline tests, 264 lines).
|===
...
7 inline `test "..."` block(s) in `iac_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `iac_ffi.zig` — C-ABI signatures and guards.
