...
// ## Return codes (ADR-0006)
// Frozen by ADR-0006. New failure modes compose these via the error JSON
// body; the integer surface does not grow without a follow-up ADR.
...
/// Compare a C-NUL-terminated tool-name pointer against a Zig string literal.
/// Caller must have already verified `tool_name` is non-null.
///
/// Implementation note (CWE-704 fix): uses `std.mem.sliceTo(ptr, 0)` which
/// scans the C string up to the first NUL — no `@ptrCast` and no `[*:0]`
/// re-typing.
...
///
/// If `body.len` exceeds the current capacity stored in `*in_out_len`, sets
/// `*in_out_len` to the required size and returns `RC_BUFFER_TOO_SMALL`; the
/// caller re-allocates and retries, per ADR-0006.
pub fn writeResult(
    out_buf: [*c]u8,
    in_out_len: [*c]usize,
    body: []const u8,
