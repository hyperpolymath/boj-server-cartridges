...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libobserve.so`.
| `observe_ffi.zig`     | C-ABI exports (12 exports, 6 inline tests, 268 lines).
|===
...
6 inline `test "..."` block(s) in `observe_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `observe_ffi.zig` — C-ABI signatures and guards.
