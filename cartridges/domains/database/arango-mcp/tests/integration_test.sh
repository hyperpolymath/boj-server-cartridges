...
# Integration tests for arango-mcp cartridge.
set -euo pipefail
...
echo "=== arango-mcp integration tests ==="
...
if command -v idris2 &>/dev/null; then
    cd "$CART_DIR/abi" && idris2 --check ArangoMcp.SafeDatabase 2>&1
    echo "  ABI: OK"
else
...
echo ""
echo "All tests passed for arango-mcp!"
