...
= supabase-mcp / ffi — Zig FFI layer
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libsupabase.so`.
| `supabase_mcp_ffi.zig`     | C-ABI exports (10 exports, 5 inline tests, 294 lines).
|===
...
5 inline `test "..."` block(s) in `supabase_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `supabase_mcp_ffi.zig` — C-ABI signatures and guards.
