...
= linear-mcp / ffi — Zig FFI layer **(stub)**
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/liblinear.so`.
| `linear_mcp_ffi.zig`     | C-ABI exports (10 exports, 5 inline tests, 461 lines).
|===
...
5 inline `test "..."` block(s) in `linear_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `linear_mcp_ffi.zig` — C-ABI signatures and guards.
