...
= observe-mcp / adapter — protocol bridge
:orientation: shallow
...
| `observe_adapter.zig` | Protocol dispatch (129 lines). Tool catalogue matches `cartridge.json`.
...
. `observe_adapter.zig` — dispatch function and per-protocol routers.
