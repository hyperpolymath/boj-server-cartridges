...
= container-mcp / adapter — protocol bridge
:orientation: shallow
...
| `container_adapter.zig` | Protocol dispatch (115 lines). Tool catalogue matches `cartridge.json`.
...
. `container_adapter.zig` — dispatch function and per-protocol routers.
