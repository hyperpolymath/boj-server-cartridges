...
defmodule PolyProof.LSP.Server do
  @moduledoc """
  GenLSP server implementation for PolyProof.
  ...
  alias PolyProof.LSP.Handlers.{Completion, Diagnostics, Hover}
  ...
  def init(_lsp, _args) do
    {:ok, %{project_path: nil, detected_prover: nil, documents: %{}}}
  end
  ...
  def handle_request(%{"method" => "initialize", "params" => params}, lsp) do
    ...
    # Auto-detect proof assistant
    detected_prover = detect_prover(project_path)

    Logger.info("Detected proof assistant: #{inspect(detected_prover)}")
    ...
    server_capabilities = %{
      ...
      "executeCommandProvider" => %{
        "commands" => ["poly-proof.check", "poly-proof.show-goals"]
      }
    }
    ...
    result = %{
      ...
      "serverInfo" => %{
        "name" => "PolyProof LSP",
        "version" => PolyProof.LSP.version()
      }
    }

    new_state = Map.merge(lsp, %{project_path: project_path, detected_prover: detected_prover})
  ...
  defp detect_prover(nil), do: nil
  defp detect_prover(project_path) do
    adapters = [
      {PolyProof.Adapters.Coq, :coq},
      {PolyProof.Adapters.Lean, :lean},
      {PolyProof.Adapters.Isabelle, :isabelle},
      {PolyProof.Adapters.Agda, :agda}
    ]
  ...
  defp execute_command("poly-proof.check", _args, %{project_path: path, detected_prover: prover}) when path != nil do
    case prover do
      :coq -> PolyProof.Adapters.Coq.check(path, [])
      :lean -> PolyProof.Adapters.Lean.check(path, [])
      :isabelle -> PolyProof.Adapters.Isabelle.check(path, [])
      :agda -> PolyProof.Adapters.Agda.check(path, [])
      _ -> {:error, "No proof assistant detected"}
    end
  end

  defp execute_command("poly-proof.show-goals", _args, %{project_path: path, detected_prover: prover}) when path != nil do
    case prover do
      :coq -> PolyProof.Adapters.Coq.show_goals(path, [])
      :lean -> PolyProof.Adapters.Lean.show_goals(path, [])
      :isabelle -> {:ok, "Isabelle uses different goal display mechanism"}
      :agda -> PolyProof.Adapters.Agda.show_goals(path, [])
      _ -> {:error, "No proof assistant detected"}
    end
  end
