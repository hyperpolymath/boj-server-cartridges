...
# Integration tests for mongodb-mcp cartridge.
set -euo pipefail
...
echo "=== mongodb-mcp integration tests ==="
...
if command -v idris2 &>/dev/null; then
    cd "$CART_DIR/abi" && idris2 --check MongodbMcp.SafeDatabase 2>&1
    echo "  ABI: OK"
else
...
echo ""
echo "All tests passed for mongodb-mcp!"
