...
= kategoria-mcp / ffi — Zig FFI layer **(stub)**
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libkategoria.so`.
| `kategoria_ffi.zig`     | C-ABI exports (4 exports, 3 inline tests, 44 lines).
|===
...
3 inline `test "..."` block(s) in `kategoria_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `kategoria_ffi.zig` — C-ABI signatures and guards.
