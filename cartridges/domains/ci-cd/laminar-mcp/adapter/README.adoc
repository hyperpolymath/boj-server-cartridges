...
= laminar-mcp / adapter — protocol bridge
:orientation: shallow
...
| `laminar_adapter.zig` | Protocol dispatch (129 lines). Tool catalogue matches `cartridge.json`.
...
. `laminar_adapter.zig` — dispatch function and per-protocol routers.
