...
# Integration tests for discord-mcp cartridge.
set -euo pipefail
...
echo "=== discord-mcp integration tests ==="
...
if command -v idris2 &>/dev/null; then
    cd "$CART_DIR/abi" && idris2 --check DiscordMcp.SafeComms 2>&1
    echo "  ABI: OK"
else
...
echo ""
echo "All tests passed for discord-mcp!"
