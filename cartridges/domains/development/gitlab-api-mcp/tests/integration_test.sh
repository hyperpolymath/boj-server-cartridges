...
# Integration tests for gitlab-api-mcp cartridge.
set -euo pipefail
...
echo "=== gitlab-api-mcp integration tests ==="
...
if command -v idris2 &>/dev/null; then
    cd "$CART_DIR/abi" && idris2 --check GitlabApiMcp.SafeCloud 2>&1
    echo "  ABI: OK"
else
...
echo ""
echo "All tests passed for gitlab-api-mcp!"
