...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libarango.so`.
| `arango_mcp_ffi.zig`     | C-ABI exports (13 exports, 7 inline tests, 374 lines).
|===
...
7 inline `test "..."` block(s) in `arango_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `arango_mcp_ffi.zig` — C-ABI signatures and guards.
