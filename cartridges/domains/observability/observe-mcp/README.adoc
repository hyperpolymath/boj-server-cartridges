...
Unified observability — metrics, logs, traces (Prometheus, Grafana, Loki, Jaeger)
...
| `observe_register` | Register an observability backend
| `observe_query_metrics` | Query metrics (PromQL)
| `observe_query_logs` | Query logs (LogQL)
| `observe_query_traces` | Query traces by service or trace ID
|===
