...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libcloud.so`.
| `cloud_ffi.zig`     | C-ABI exports (47 exports, 19 inline tests, 1204 lines).
|===
...
19 inline `test "..."` block(s) in `cloud_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `cloud_ffi.zig` — C-ABI signatures and guards.
