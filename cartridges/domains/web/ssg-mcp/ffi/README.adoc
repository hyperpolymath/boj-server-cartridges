...
| `ssg_ffi.zig`     | C-ABI exports (13 exports, 7 inline tests, 290 lines).
|===
...
7 inline `test "..."` block(s) in `ssg_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `ssg_ffi.zig` — C-ABI signatures and guards.
