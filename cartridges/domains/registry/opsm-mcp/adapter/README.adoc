...
= opsm-mcp / adapter — protocol bridge
:orientation: shallow
...
| `opsm_adapter.zig` | Protocol dispatch (365 lines). Tool catalogue matches `cartridge.json`.
...
. `opsm_adapter.zig` — dispatch function and per-protocol routers.
