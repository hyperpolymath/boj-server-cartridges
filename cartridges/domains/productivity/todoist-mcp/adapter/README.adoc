...
= todoist-mcp / adapter — protocol bridge
:orientation: shallow
...
| `todoist_adapter.zig` | Protocol dispatch (204 lines). Tool catalogue matches `cartridge.json`.
...
. `todoist_adapter.zig` — dispatch function and per-protocol routers.
