...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libneo4j.so`.
| `neo4j_mcp_ffi.zig`     | C-ABI exports (11 exports, 6 inline tests, 350 lines).
|===
...
6 inline `test "..."` block(s) in `neo4j_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `neo4j_mcp_ffi.zig` — C-ABI signatures and guards.
