...
//
// laminar-mcp/adapter/laminar_adapter.zig
//
// Three-protocol BoJ adapter: REST (port 9259), gRPC-compat (port 9260),
// GraphQL (port 9261).
// Replaces the banned zig adapter (laminar_adapter.v).

const std = @import("std");
const ffi = @import("laminar_ffi");

const REST_PORT: u16 = 9259;
const GRPC_PORT: u16 = 9260;
const GQL_PORT:  u16 = 9261;
...
fn dispatch(tool: []const u8, _body: []const u8, resp: []u8) Response {
    _ = _body;
    if (std.mem.eql(u8, tool, "laminar_create_pipeline")) {
        return .{ .status = 200, .body = okJson(resp, "laminar_create_pipeline forwarded") };
    }
    if (std.mem.eql(u8, tool, "laminar_run_stage")) {
        return .{ .status = 200, .body = okJson(resp, "laminar_run_stage forwarded") };
    }
    if (std.mem.eql(u8, tool, "laminar_get_status")) {
        return .{ .status = 200, .body = okJson(resp, "laminar_get_status forwarded") };
    }
    if (std.mem.eql(u8, tool, "laminar_cancel_pipeline")) {
        return .{ .status = 200, .body = okJson(resp, "laminar_cancel_pipeline forwarded") };
    }
...
fn dispatchGraphql(body: []const u8, resp: []u8) Response {
    if (std.mem.indexOf(u8, body, "laminar_create_pipeline") != null)
        return dispatch("laminar_create_pipeline", body, resp);
    if (std.mem.indexOf(u8, body, "laminar_run_stage") != null)
        return dispatch("laminar_run_stage", body, resp);
    if (std.mem.indexOf(u8, body, "laminar_get_status") != null)
        return dispatch("laminar_get_status", body, resp);
    if (std.mem.indexOf(u8, body, "laminar_cancel_pipeline") != null)
        return dispatch("laminar_cancel_pipeline", body, resp);
