...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libpmpl.so`.
| `pmpl_ffi.zig`     | C-ABI exports (6 exports, 2 inline tests, 75 lines).
|===
...
2 inline `test "..."` block(s) in `pmpl_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `pmpl_ffi.zig` — C-ABI signatures and guards.
