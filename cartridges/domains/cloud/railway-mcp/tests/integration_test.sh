...
# Integration tests for railway-mcp cartridge.
set -euo pipefail
...
echo "=== railway-mcp integration tests ==="
...
if command -v idris2 &>/dev/null; then
    cd "$CART_DIR/abi" && idris2 --check RailwayMcp.SafeCloud 2>&1
    echo "  ABI: OK"
else
...
echo ""
echo "All tests passed for railway-mcp!"
