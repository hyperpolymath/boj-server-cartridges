...
//
// secrets-mcp/adapter/secrets_adapter.zig
//
// Three-protocol BoJ adapter: REST (port 9289), gRPC-compat (port 9290),
// GraphQL (port 9291).
// Replaces the banned zig adapter (secrets_adapter.v).

const std = @import("std");
const ffi = @import("secrets_ffi");

const REST_PORT: u16 = 9289;
const GRPC_PORT: u16 = 9290;
const GQL_PORT:  u16 = 9291;
...
fn dispatch(tool: []const u8, _body: []const u8, resp: []u8) Response {
    _ = _body;
    if (std.mem.eql(u8, tool, "secrets_unseal")) {
        return .{ .status = 200, .body = okJson(resp, "secrets_unseal forwarded") };
    }
    if (std.mem.eql(u8, tool, "secrets_authenticate")) {
        return .{ .status = 200, .body = okJson(resp, "secrets_authenticate forwarded") };
    }
    if (std.mem.eql(u8, tool, "secrets_get")) {
        return .{ .status = 200, .body = okJson(resp, "secrets_get forwarded") };
    }
    if (std.mem.eql(u8, tool, "secrets_set")) {
        return .{ .status = 200, .body = okJson(resp, "secrets_set forwarded") };
    }
    if (std.mem.eql(u8, tool, "secrets_seal")) {
        return .{ .status = 200, .body = okJson(resp, "secrets_seal forwarded") };
    }
...
fn dispatchGraphql(body: []const u8, resp: []u8) Response {
    if (std.mem.indexOf(u8, body, "secrets_unseal") != null)
        return dispatch("secrets_unseal", body, resp);
    if (std.mem.indexOf(u8, body, "secrets_authenticate") != null)
        return dispatch("secrets_authenticate", body, resp);
    if (std.mem.indexOf(u8, body, "secrets_get") != null)
        return dispatch("secrets_get", body, resp);
    if (std.mem.indexOf(u8, body, "secrets_set") != null)
        return dispatch("secrets_set", body, resp);
    if (std.mem.indexOf(u8, body, "secrets_seal") != null)
        return dispatch("secrets_seal", body, resp);
