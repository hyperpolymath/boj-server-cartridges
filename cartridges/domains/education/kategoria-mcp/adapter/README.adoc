...
= kategoria-mcp / adapter — protocol bridge
:orientation: shallow
...
| `kategoria_adapter.zig` | Protocol dispatch (118 lines). Tool catalogue matches `cartridge.json`.
...
. `kategoria_adapter.zig` — dispatch function and per-protocol routers.
