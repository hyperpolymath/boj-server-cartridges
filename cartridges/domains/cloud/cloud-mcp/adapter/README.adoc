...
= cloud-mcp / adapter — protocol bridge
:orientation: shallow
...
| `cloud_adapter.zig` | Protocol dispatch (129 lines). Tool catalogue matches `cartridge.json`.
...
. `cloud_adapter.zig` — dispatch function and per-protocol routers.
