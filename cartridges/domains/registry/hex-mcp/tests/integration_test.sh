...
# Integration tests for hex-mcp cartridge.
set -euo pipefail
...
echo "=== hex-mcp integration tests ==="
...
if command -v idris2 &>/dev/null; then
    cd "$CART_DIR/abi" && idris2 --check HexMcp.SafeRegistry 2>&1
    echo "  ABI: OK"
else
...
echo ""
echo "All tests passed for hex-mcp!"
