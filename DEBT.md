...
| C-7 | MEDIUM | **13 of `cicd-suite`'s 26 actions cannot fail** — `::warning::` then exit 0, while named "Gate". *(Was HIGH and three-part; cicd-suite#1 fixed `code-hygiene-check`'s whole-tree grep — 313 matching files here, now 3 true positives — and `required-files-check`'s presence-only filler, and ended their mutual contradiction. The advisory/enforcing split remains.)* The consuming workflow sits in **199 repos**, untracked in **198**. | `for a in ../cicd-suite/actions/*/; do grep -q 'exit 1' $a/action.yml \|\| echo $a; done \| wc -l` → 13 |
