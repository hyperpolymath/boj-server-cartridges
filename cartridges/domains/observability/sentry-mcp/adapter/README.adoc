...
= sentry-mcp / adapter — protocol bridge
:orientation: shallow
...
| `sentry_adapter.zig` | Protocol dispatch (204 lines). Tool catalogue matches `cartridge.json`.
...
. `sentry_adapter.zig` — dispatch function and per-protocol routers.
