...
= research-mcp / ffi — Zig FFI layer
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libresearch.so`.
| `research_ffi.zig`     | C-ABI exports (19 exports, 12 inline tests, 463 lines).
|===
...
12 inline `test "..."` block(s) in `research_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `research_ffi.zig` — C-ABI signatures and guards.
