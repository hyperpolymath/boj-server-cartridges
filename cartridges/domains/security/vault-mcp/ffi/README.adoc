...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libvault.so`.
| `vault_mcp_ffi.zig`     | C-ABI exports (18 exports, 10 inline tests, 680 lines).
|===
...
10 inline `test "..."` block(s) in `vault_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `vault_mcp_ffi.zig` — C-ABI signatures and guards.
