...
= feedback-mcp / adapter — protocol bridge
:orientation: shallow
...
| `feedback_adapter.zig` | Protocol dispatch (129 lines). Tool catalogue matches `cartridge.json`.
...
. `feedback_adapter.zig` — dispatch function and per-protocol routers.
