...
= dap-mcp / adapter — protocol bridge
:orientation: shallow
...
| `dap_adapter.zig` | Protocol dispatch (403 lines). Tool catalogue matches `cartridge.json`.
...
. `dap_adapter.zig` — dispatch function and per-protocol routers.
