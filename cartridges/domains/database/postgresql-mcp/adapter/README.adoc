...
= postgresql-mcp / adapter — protocol bridge
:orientation: shallow
...
| `postgresql_mcp_adapter.zig` | Protocol dispatch (118 lines). Tool catalogue matches `cartridge.json`.
...
. `postgresql_mcp_adapter.zig` — dispatch function and per-protocol routers.
