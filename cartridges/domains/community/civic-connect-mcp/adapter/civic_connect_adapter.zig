...
const REST_PORT: u16 = 9208;
const GRPC_PORT: u16 = 9209;
const GQL_PORT:  u16 = 9210;
...
fn dispatch(tool: []const u8, _body: []const u8, resp: []u8) Response {
    _ = _body;
    if (std.mem.eql(u8, tool, "civic_list_channels")) {
        return .{ .status = 200, .body = okJson(resp, "civic_list_channels forwarded") };
    }
    if (std.mem.eql(u8, tool, "civic_send_message")) {
        return .{ .status = 200, .body = okJson(resp, "civic_send_message forwarded") };
    }
    if (std.mem.eql(u8, tool, "civic_get_poll")) {
        return .{ .status = 200, .body = okJson(resp, "civic_get_poll forwarded") };
    }
...
fn dispatchGraphql(body: []const u8, resp: []u8) Response {
    if (std.mem.indexOf(u8, body, "civic_list_channels") != null)
        return dispatch("civic_list_channels", body, resp);
    if (std.mem.indexOf(u8, body, "civic_send_message") != null)
        return dispatch("civic_send_message", body, resp);
    if (std.mem.indexOf(u8, body, "civic_get_poll") != null)
        return dispatch("civic_get_poll", body, resp);
