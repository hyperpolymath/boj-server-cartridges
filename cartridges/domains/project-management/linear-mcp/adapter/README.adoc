...
= linear-mcp / adapter — protocol bridge
:orientation: shallow
...
| `linear_mcp_adapter.zig` | Protocol dispatch (144 lines). Tool catalogue matches `cartridge.json`.
...
. `linear_mcp_adapter.zig` — dispatch function and per-protocol routers.
