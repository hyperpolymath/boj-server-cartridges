...
//
// observe-mcp/adapter/observe_adapter.zig
//
// Three-protocol BoJ adapter: REST (port 9274), gRPC-compat (port 9275),
// GraphQL (port 9276).
// Replaces the banned zig adapter (observe_adapter.v).

const std = @import("std");
const ffi = @import("observe_ffi");

const REST_PORT: u16 = 9274;
const GRPC_PORT: u16 = 9275;
const GQL_PORT:  u16 = 9276;
...
fn dispatch(tool: []const u8, _body: []const u8, resp: []u8) Response {
    _ = _body;
    if (std.mem.eql(u8, tool, "observe_register")) {
        return .{ .status = 200, .body = okJson(resp, "observe_register forwarded") };
    }
    if (std.mem.eql(u8, tool, "observe_query_metrics")) {
        return .{ .status = 200, .body = okJson(resp, "observe_query_metrics forwarded") };
    }
    if (std.mem.eql(u8, tool, "observe_query_logs")) {
        return .{ .status = 200, .body = okJson(resp, "observe_query_logs forwarded") };
    }
    if (std.mem.eql(u8, tool, "observe_query_traces")) {
        return .{ .status = 200, .body = okJson(resp, "observe_query_traces forwarded") };
    }
...
fn dispatchGraphql(body: []const u8, resp: []u8) Response {
    if (std.mem.indexOf(u8, body, "observe_register") != null)
        return dispatch("observe_register", body, resp);
    if (std.mem.indexOf(u8, body, "observe_query_metrics") != null)
        return dispatch("observe_query_metrics", body, resp);
    if (std.mem.indexOf(u8, body, "observe_query_logs") != null)
        return dispatch("observe_query_logs", body, resp);
    if (std.mem.indexOf(u8, body, "observe_query_traces") != null)
        return dispatch("observe_query_traces", body, resp);
