...
= turso-mcp / adapter — protocol bridge
:orientation: shallow
...
| `turso_mcp_adapter.zig` | Protocol dispatch (112 lines). Tool catalogue matches `cartridge.json`.
...
. `turso_mcp_adapter.zig` — dispatch function and per-protocol routers.
