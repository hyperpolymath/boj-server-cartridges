...
= burble-admin-mcp / adapter — protocol bridge
:orientation: shallow
...
| `burble_admin_adapter.zig` | Protocol dispatch (159 lines). Tool catalogue matches `cartridge.json`.
...
. `burble_admin_adapter.zig` — dispatch function and per-protocol routers.
