...
= github-api-mcp / adapter — protocol bridge
:orientation: shallow
...
| `github_api_mcp_adapter.zig` | Protocol dispatch (149 lines). Tool catalogue matches `cartridge.json`.
...
. `github_api_mcp_adapter.zig` — dispatch function and per-protocol routers.
