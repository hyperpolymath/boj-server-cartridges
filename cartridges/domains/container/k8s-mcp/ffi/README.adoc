...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libk8s.so`.
| `k8s_ffi.zig`     | C-ABI exports (12 exports, 8 inline tests, 299 lines).
|===
...
8 inline `test "..."` block(s) in `k8s_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `k8s_ffi.zig` — C-ABI signatures and guards.
