...
| ID | Sev | Item | Evidence |
|----|-----|------|----------|
| C-1 | HIGH | **`.github/workflows/main-estate-audit.yml` references a repository that does not exist.** All 26 gates are `hyperpolymath/cicd-suite/actions/*@main`, and **`hyperpolymath/cicd-suite` returns 404** — anonymously, and to a token that can see all 35 private repos in the account. Also untracked, no `permissions:` block, `actions/checkout` pinned by tag. Never run in either repo. **It cannot be SHA-pinned — there is nothing to pin to.** Needs an owner decision (delete, or create `cicd-suite`), not a pinning sweep. Identical file in the sibling repo. | `gh api repos/hyperpolymath/cicd-suite` → 404 |
