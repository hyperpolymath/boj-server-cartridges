...
= airtable-mcp / ffi — Zig FFI layer
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libairtable.so`.
| `airtable_mcp_ffi.zig`     | C-ABI exports (15 exports, 5 inline tests, 333 lines).
|===
...
5 inline `test "..."` block(s) in `airtable_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `airtable_mcp_ffi.zig` — C-ABI signatures and guards.
