...
# Integration tests for telegram-mcp cartridge.
set -euo pipefail
...
echo "=== telegram-mcp integration tests ==="
...
if command -v idris2 &>/dev/null; then
    cd "$CART_DIR/abi" && idris2 --check TelegramMcp.SafeComms 2>&1
    echo "  ABI: OK"
else
...
echo ""
echo "All tests passed for telegram-mcp!"
