...
pub fn build(b: *std.Build) void {
    ...
    const ffi_mod = b.createModule(.{
        .root_source_file = b.path("../ffi/conflow_ffi.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const adapter_mod = b.createModule(.{
        .root_source_file = b.path("conflow_adapter.zig"),
        .target = target,
        .optimize = optimize,
    });
    adapter_mod.addImport("conflow_ffi", ffi_mod);

    const adapter = b.addExecutable(.{
        .name = "conflow_adapter",
        .root_module = adapter_mod,
    });
    b.installArtifact(adapter);

    const run_step = b.step("run", "Run the conflow-mcp adapter");
    ...
    const test_step = b.step("test", "Run conflow-mcp adapter tests");
