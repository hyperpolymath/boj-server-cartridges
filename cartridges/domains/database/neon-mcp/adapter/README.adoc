...
= neon-mcp / adapter — protocol bridge
:orientation: shallow
...
| `neon_mcp_adapter.zig` | Protocol dispatch (112 lines). Tool catalogue matches `cartridge.json`.
...
. `neon_mcp_adapter.zig` — dispatch function and per-protocol routers.
