...
= telegram-mcp / ffi — Zig FFI layer
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libtelegram.so`.
| `telegram_mcp_ffi.zig`     | C-ABI exports (14 exports, 6 inline tests, 374 lines).
|===
...
6 inline `test "..."` block(s) in `telegram_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `telegram_mcp_ffi.zig` — C-ABI signatures and guards.
