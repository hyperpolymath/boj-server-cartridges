...
# Integration tests for google-sheets-mcp cartridge.
set -euo pipefail
...
echo "=== google-sheets-mcp integration tests ==="
...
if command -v idris2 &>/dev/null; then
    cd "$CART_DIR/abi" && idris2 --check GoogleSheetsMcp.SafeRegistry 2>&1
    echo "  ABI: OK"
else
...
echo ""
echo "All tests passed for google-sheets-mcp!"
