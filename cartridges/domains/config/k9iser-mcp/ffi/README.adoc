...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libssg.so`.
| `k9iser_ffi.zig`     | C-ABI exports (13 exports, 7 inline tests, 290 lines).
|===
...
7 inline `test "..."` block(s) in `k9iser_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `k9iser_ffi.zig` — C-ABI signatures and guards.
