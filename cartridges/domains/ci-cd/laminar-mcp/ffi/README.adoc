...
= laminar-mcp / ffi — Zig FFI layer **(stub)**
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/liblaminar.so`.
| `laminar_ffi.zig`     | C-ABI exports (4 exports, 3 inline tests, 44 lines).
|===
...
3 inline `test "..."` block(s) in `laminar_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `laminar_ffi.zig` — C-ABI signatures and guards.
