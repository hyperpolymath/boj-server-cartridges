...
= ssg-mcp / adapter — protocol bridge
:orientation: shallow
...
| `ssg_adapter.zig` | Protocol dispatch (134 lines). Tool catalogue matches `cartridge.json`.
...
. `ssg_adapter.zig` — dispatch function and per-protocol routers.
