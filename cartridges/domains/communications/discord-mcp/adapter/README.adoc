...
= discord-mcp / adapter — protocol bridge
:orientation: shallow
...
| `discord_mcp_adapter.zig` | Protocol dispatch (112 lines). Tool catalogue matches `cartridge.json`.
...
. `discord_mcp_adapter.zig` — dispatch function and per-protocol routers.
