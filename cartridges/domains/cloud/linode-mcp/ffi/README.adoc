...
| `build.zig`      | Zig build graph — produces `zig-out/lib/liblinode.so`.
| `linode_mcp_ffi.zig`     | C-ABI exports (10 exports, 6 inline tests, 319 lines).
|===
...
6 inline `test "..."` block(s) in `linode_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `linode_mcp_ffi.zig` — C-ABI signatures and guards.
