...
= zotero-mcp / adapter — protocol bridge
:orientation: shallow
...
| `zotero_adapter.zig` | Protocol dispatch (212 lines). Tool catalogue matches `cartridge.json`.
...
. `zotero_adapter.zig` — dispatch function and per-protocol routers.
