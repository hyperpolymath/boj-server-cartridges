...
//
// rokur-mcp/adapter/rokur_mcp_adapter.zig
//
// Three-protocol BoJ adapter: REST (port 9286), gRPC-compat (port 9287),
// GraphQL (port 9288).
// Replaces the banned zig adapter (rokur_mcp_adapter.v).

const std = @import("std");
const ffi = @import("rokur_mcp_ffi");

const REST_PORT: u16 = 9286;
const GRPC_PORT: u16 = 9287;
const GQL_PORT:  u16 = 9288;
...
fn dispatch(tool: []const u8, _body: []const u8, resp: []u8) Response {
    _ = _body;
    if (std.mem.eql(u8, tool, "rokur_authorize")) {
        return .{ .status = 200, .body = okJson(resp, "rokur_authorize forwarded") };
    }
    if (std.mem.eql(u8, tool, "rokur_health")) {
        return .{ .status = 200, .body = okJson(resp, "rokur_health forwarded") };
    }
    if (std.mem.eql(u8, tool, "rokur_secrets_status")) {
        return .{ .status = 200, .body = okJson(resp, "rokur_secrets_status forwarded") };
    }
    if (std.mem.eql(u8, tool, "rokur_reload")) {
        return .{ .status = 200, .body = okJson(resp, "rokur_reload forwarded") };
    }
...
fn dispatchGraphql(body: []const u8, resp: []u8) Response {
    if (std.mem.indexOf(u8, body, "rokur_authorize") != null)
        return dispatch("rokur_authorize", body, resp);
    if (std.mem.indexOf(u8, body, "rokur_health") != null)
        return dispatch("rokur_health", body, resp);
    if (std.mem.indexOf(u8, body, "rokur_secrets_status") != null)
        return dispatch("rokur_secrets_status", body, resp);
    if (std.mem.indexOf(u8, body, "rokur_reload") != null)
        return dispatch("rokur_reload", body, resp);
