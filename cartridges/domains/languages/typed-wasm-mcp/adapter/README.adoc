...
= typed-wasm-mcp / adapter — protocol bridge
:orientation: shallow
...
| `typed_wasm_adapter.zig` | Protocol dispatch (118 lines). Tool catalogue matches `cartridge.json`.
...
. `typed_wasm_adapter.zig` — dispatch function and per-protocol routers.
