...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libqueues.so`.
| `queues_ffi.zig`     | C-ABI exports (15 exports, 7 inline tests, 310 lines).
|===
...
7 inline `test "..."` block(s) in `queues_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `queues_ffi.zig` — C-ABI signatures and guards.
