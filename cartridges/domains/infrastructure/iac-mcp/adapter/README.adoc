...
= iac-mcp / adapter — protocol bridge
:orientation: shallow
...
| `iac_adapter.zig` | Protocol dispatch (112 lines). Tool catalogue matches `cartridge.json`.
...
. `iac_adapter.zig` — dispatch function and per-protocol routers.
