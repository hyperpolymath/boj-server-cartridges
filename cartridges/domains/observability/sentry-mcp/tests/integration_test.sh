...
# Integration tests for sentry-mcp cartridge.
set -euo pipefail
...
echo "=== sentry-mcp integration tests ==="
...
if command -v idris2 &>/dev/null; then
    cd "$CART_DIR/abi" && idris2 --check SentryMcp.SafeRegistry 2>&1
    echo "  ABI: OK"
else
...
echo ""
echo "All tests passed for sentry-mcp!"
