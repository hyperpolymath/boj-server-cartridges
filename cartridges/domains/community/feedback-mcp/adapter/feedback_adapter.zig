...
//
// feedback-mcp/adapter/feedback_adapter.zig
//
// Three-protocol BoJ adapter: REST (port 9232), gRPC-compat (port 9233),
// GraphQL (port 9234).
// Replaces the banned zig adapter (feedback_adapter.v).

const std = @import("std");
const ffi = @import("feedback_ffi");

const REST_PORT: u16 = 9232;
const GRPC_PORT: u16 = 9233;
const GQL_PORT:  u16 = 9234;
...
fn dispatch(tool: []const u8, _body: []const u8, resp: []u8) Response {
    _ = _body;
    if (std.mem.eql(u8, tool, "feedback_register_channel")) {
        return .{ .status = 200, .body = okJson(resp, "feedback_register_channel forwarded") };
    }
    if (std.mem.eql(u8, tool, "feedback_start_collecting")) {
        return .{ .status = 200, .body = okJson(resp, "feedback_start_collecting forwarded") };
    }
    if (std.mem.eql(u8, tool, "feedback_submit")) {
        return .{ .status = 200, .body = okJson(resp, "feedback_submit forwarded") };
    }
    if (std.mem.eql(u8, tool, "feedback_get_stats")) {
        return .{ .status = 200, .body = okJson(resp, "feedback_get_stats forwarded") };
    }
...
fn dispatchGraphql(body: []const u8, resp: []u8) Response {
    if (std.mem.indexOf(u8, body, "feedback_register_channel") != null)
        return dispatch("feedback_register_channel", body, resp);
    if (std.mem.indexOf(u8, body, "feedback_start_collecting") != null)
        return dispatch("feedback_start_collecting", body, resp);
    if (std.mem.indexOf(u8, body, "feedback_submit") != null)
        return dispatch("feedback_submit", body, resp);
    if (std.mem.indexOf(u8, body, "feedback_get_stats") != null)
        return dispatch("feedback_get_stats", body, resp);
