...
pub fn build(b: *std.Build) void {
    ...
    const ffi_mod = b.createModule(.{
        .root_source_file = b.path("../ffi/iac_ffi.zig"),
        .target = target,
        .optimize = optimize,
    });

    const adapter_mod = b.createModule(.{
        .root_source_file = b.path("iac_adapter.zig"),
        .target = target,
        .optimize = optimize,
    });
    adapter_mod.addImport("iac_ffi", ffi_mod);
    ...
    const run_step = b.step("run", "Run the iac-mcp adapter");
    ...
    const test_step = b.step("test", "Run iac-mcp adapter tests");
