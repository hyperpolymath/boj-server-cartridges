...
= github-api-mcp / ffi — Zig FFI layer
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libgithub-api.so`.
| `github_api_mcp_ffi.zig`     | C-ABI exports (17 exports, 8 inline tests, 707 lines).
|===
...
8 inline `test "..."` block(s) in `github_api_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `github_api_mcp_ffi.zig` — C-ABI signatures and guards.
