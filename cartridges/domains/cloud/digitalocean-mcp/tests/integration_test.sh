...
# Integration tests for digitalocean-mcp cartridge.
set -euo pipefail
...
echo "=== digitalocean-mcp integration tests ==="
...
if command -v idris2 &>/dev/null; then
    cd "$CART_DIR/abi" && idris2 --check DigitaloceanMcp.SafeCloud 2>&1
    echo "  ABI: OK"
else
...
echo ""
echo "All tests passed for digitalocean-mcp!"
