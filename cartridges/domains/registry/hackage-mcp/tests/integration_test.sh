...
# Integration tests for hackage-mcp cartridge.
set -euo pipefail
...
echo "=== hackage-mcp integration tests ==="
...
if command -v idris2 &>/dev/null; then
    cd "$CART_DIR/abi" && idris2 --check HackageMcp.SafeRegistry 2>&1
    echo "  ABI: OK"
else
...
echo ""
echo "All tests passed for hackage-mcp!"
