...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libdap.so`.
| `dap_ffi.zig`     | C-ABI exports (18 exports, 5 inline tests, 348 lines).
|===
...
5 inline `test "..."` block(s) in `dap_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `dap_ffi.zig` — C-ABI signatures and guards.
