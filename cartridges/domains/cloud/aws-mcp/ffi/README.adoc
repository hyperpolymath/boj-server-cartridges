...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libaws.so`.
| `aws_mcp_ffi.zig`     | C-ABI exports (14 exports, 9 inline tests, 465 lines).
|===
...
9 inline `test "..."` block(s) in `aws_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `aws_mcp_ffi.zig` — C-ABI signatures and guards.
