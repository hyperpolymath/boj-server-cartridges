...
= civic-connect-mcp / adapter — protocol bridge
:orientation: shallow
...
| `civic_connect_adapter.zig` | Protocol dispatch (124 lines). Tool catalogue matches `cartridge.json`.
...
. `civic_connect_adapter.zig` — dispatch function and per-protocol routers.
