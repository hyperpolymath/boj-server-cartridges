...
| `build.zig`      | Zig build graph — produces `zig-out/lib/librailway.so`.
| `railway_mcp_ffi.zig`     | C-ABI exports (17 exports, 6 inline tests, 389 lines).
|===
...
6 inline `test "..."` block(s) in `railway_mcp_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `railway_mcp_ffi.zig` — C-ABI signatures and guards.
