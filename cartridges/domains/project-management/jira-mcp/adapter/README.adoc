...
= jira-mcp / adapter — protocol bridge
:orientation: shallow
...
| `jira_mcp_adapter.zig` | Protocol dispatch (149 lines). Tool catalogue matches `cartridge.json`.
...
. `jira_mcp_adapter.zig` — dispatch function and per-protocol routers.
