...
const REST_PORT: u16 = 9211;
const GRPC_PORT: u16 = 9212;
const GQL_PORT:  u16 = 9213;
...
fn dispatch(tool: []const u8, _body: []const u8, resp: []u8) Response {
    _ = _body;
    if (std.mem.eql(u8, tool, "cloud_authenticate")) {
        return .{ .status = 200, .body = okJson(resp, "cloud_authenticate forwarded") };
    }
    if (std.mem.eql(u8, tool, "cloud_logout")) {
        return .{ .status = 200, .body = okJson(resp, "cloud_logout forwarded") };
    }
    if (std.mem.eql(u8, tool, "cloud_state")) {
        return .{ .status = 200, .body = okJson(resp, "cloud_state forwarded") };
    }
    if (std.mem.eql(u8, tool, "cloud_execute")) {
        return .{ .status = 200, .body = okJson(resp, "cloud_execute forwarded") };
    }
...
fn dispatchGraphql(body: []const u8, resp: []u8) Response {
    if (std.mem.indexOf(u8, body, "cloud_authenticate") != null)
        return dispatch("cloud_authenticate", body, resp);
    if (std.mem.indexOf(u8, body, "cloud_logout") != null)
        return dispatch("cloud_logout", body, resp);
    if (std.mem.indexOf(u8, body, "cloud_state") != null)
        return dispatch("cloud_state", body, resp);
    if (std.mem.indexOf(u8, body, "cloud_execute") != null)
        return dispatch("cloud_execute", body, resp);
