...
= feedback-mcp / ffi — Zig FFI layer
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libfeedback.so`.
| `feedback_ffi.zig`     | C-ABI exports (15 exports, 5 inline tests, 310 lines).
|===
...
5 inline `test "..."` block(s) in `feedback_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `feedback_ffi.zig` — C-ABI signatures and guards.
