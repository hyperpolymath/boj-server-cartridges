...
= vordr-mcp / ffi — Zig FFI layer **(stub)**
:orientation: shallow
...
| `build.zig`      | Zig build graph — produces `zig-out/lib/libvordr.so`.
| `vordr_ffi.zig`     | C-ABI exports (5 exports, 2 inline tests, 66 lines).
|===
...
2 inline `test "..."` block(s) in `vordr_ffi.zig`. Run with
`cd ffi && zig build test`.

== Read-first

. `vordr_ffi.zig` — C-ABI signatures and guards.
